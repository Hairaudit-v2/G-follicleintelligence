/**
 * FI-TRICHOSCOPY-1B-CERT — Live staging certification harness (consultation integration).
 *
 * Modes:
 *   --preflight                 Check env / stub / 1B migration expectation; never mutate staging
 *   --init-run                  Create runs/<run-id>/ from 1B templates
 *   --execute-security-probes   POST invalid / expired / cross-tenant events to staging
 *
 * GREEN clinical consultation round-trip is operator-supervised; this harness scaffolds
 * evidence and certifies inbound auth probes shared with 1A.
 *
 * Usage:
 *   npm run certify:trichoscopy-1b -- --preflight
 *   npm run certify:trichoscopy-1b -- --init-run
 *   npm run certify:trichoscopy-1b -- --execute-security-probes
 */
...
const REQUIRED_MIGRATIONS = [
  ...
  "20261109120001_fi_hli_trichoscopy_consultation_1b.sql",
] as const;
...
function runPreflight(): boolean {
  ...
  console.log("\n=== FI-TRICHOSCOPY-1B-CERT preflight ===\n");
  console.log(
    JSON.stringify(
      {
        ...
        consultationIdPresent: Boolean(cfg.consultationId),
  ...
  record(
    "P5_1b_migration_declared",
    true,
    "Operator must confirm 20261109120001 applied on staging DB before GREEN"
  );
  record(
    "P6_tenants",
  ...
  record(
    "P7_consultation_optional",
    true,
    cfg.consultationId
      ? `FI_TRICHOSCOPY_CERT_CONSULTATION_ID=${cfg.consultationId}`
      : "optional until live journey; set FI_TRICHOSCOPY_CERT_CONSULTATION_ID when known"
  );
...
const ARTIFACT_TEMPLATES = [
  ...
  "hli-event-sequence.json",
  "imported-findings.json",
  "finding-reviews.json",
  "decision-links.json",
  "evidence-pack-lineage.json",
  "reconciliation.json",
  "authorisation-results.json",
  "replay-results.json",
] as const;

function initRunFolder(): string {
  const stamp = new Date().toISOString().replace(/[:.]/g, "").slice(0, 15);
  const runId = `${stamp}-1b-staging`;
  const runDir = resolve(process.cwd(), "docs/fios/trichoscopy/evidence/runs", runId);
  const templateDir = resolve(process.cwd(), "docs/fios/trichoscopy/evidence/templates/1b");
  mkdirSync(resolve(runDir, "screenshots"), { recursive: true });

  const manifestTemplate = resolve(templateDir, "certification-run-manifest.template.json");
  const manifestPath = resolve(runDir, "manifest.json");
  copyFileSync(manifestTemplate, manifestPath);

  for (const name of ARTIFACT_TEMPLATES) {
    copyFileSync(resolve(templateDir, name), resolve(runDir, name));
  }
  copyFileSync(
    resolve(templateDir, "screenshots/README.md"),
    resolve(runDir, "screenshots/README.md")
  );
  ...
  writeFileSync(
    ...
    `# Run ${runId}\n\nComplete artifact JSON files during the live FI-TRICHOSCOPY-1B consultation journey. Commit only redacted artifacts.\n\nCanonical checklist: \`docs/fios/trichoscopy/evidence/FI-TRICHOSCOPY-1B-CERT.md\`\n`,
    "utf8"
  );
...
async function executeSecurityProbes(): Promise<boolean> {
  const cfg = loadConfig();
  console.log("\n=== FI-TRICHOSCOPY-1B-CERT security probes ===\n");
  ...
  {
    const body = JSON.stringify(syntheticEnvelope(cfg.tenantId, `inv-1b-${randomUUID()}`));
    ...
    record("N6_invalid_signature", pass, `status=${res.status} reason=${String(res.json.reason ?? "none")}`);
  }

  {
    const body = JSON.stringify(syntheticEnvelope(cfg.tenantId, `exp-1b-${randomUUID()}`));
    const headers = buildHeaders({
      tenantId: cfg.tenantId,
      secret: cfg.webhookSecret,
      body,
    ...
    record("N7_expired_timestamp", pass, `status=${res.status} reason=${String(res.json.reason ?? "none")}`);
  }

  {
    const body = JSON.stringify(syntheticEnvelope(cfg.otherTenantId, `xt-1b-${randomUUID()}`));
    const headers = buildHeaders({
      tenantId: cfg.tenantId,
      secret: cfg.webhookSecret,
      body,
    ...
    record("N4_cross_tenant", pass, `status=${res.status} reason=${String(res.json.reason ?? "none")}`);
  }

  return results.filter((r) => r.id.startsWith("N")).every((r) => r.ok);
}
...
async function main(): Promise<void> {
  ...
  if (wantInit && !wantPreflight && !wantProbes) {
    initRunFolder();
    console.log("\nRun folder ready. Fill artifacts during the live consultation journey.");
    return;
  }
  ...
  if (!exitOk || failed.length) {
    console.error(
      "\nVerdict remains AMBER until live consultation evidence is committed under docs/fios/trichoscopy/evidence/runs/."
    );
    process.exit(1);
  }
  console.log(
    "\nPreflight/probes OK. Complete the supervised consultation journey (see FI-TRICHOSCOPY-1B-CERT.md) before flipping GREEN."
  );
}
