/**
 * Apply fi_case_post_op_tracking + fi_case_follow_ups migration to hosted Supabase
 * when CLI link/login is unavailable.
 * Usage: SUPABASE_DB_PASSWORD='...' node scripts/apply-fi-case-post-op-tracking-remote.mjs
 */
...
const MIGRATION_VERSION = "20260618120001";
const MIGRATION_NAME = "fi_case_post_op_tracking";
...
const sql = readFileSync(
  join(root, "supabase/migrations/20260618120001_fi_case_post_op_tracking.sql"),
  "utf8"
);
...
async function main() {
  ...
  try {
    const exists = await client.query(
      `select 1 from information_schema.tables where table_schema = 'public' and table_name = 'fi_case_post_op_tracking'`
    );
    if (exists.rowCount > 0) {
      console.log("fi_case_post_op_tracking already exists — skipping DDL.");
    } else {
      console.log("Applying migration SQL…");
      await client.query(sql);
      console.log("fi_case_post_op_tracking and fi_case_follow_ups created.");
    }
