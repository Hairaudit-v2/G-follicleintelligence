#!/usr/bin/env tsx
/**
 * Reclassify harsh@evolvedhair.com.au from CFO (finance-adjacent staff label) to clinic Admin.
 * Dry-run by default; pass --commit to apply.
 *
 *   node scripts/run-with-system-ca.mjs tsx scripts/reclassify-evolved-harsh-cfo-to-clinic-admin.ts
 *   node scripts/run-with-system-ca.mjs tsx scripts/reclassify-evolved-harsh-cfo-to-clinic-admin.ts --commit
 */
...
async function main(): Promise<void> {
  ...
  if (before.auth?.fi_tenant_id && before.auth.fi_tenant_id !== TENANT_ID) {
    console.warn(
      `\nWARN: auth fi_tenant_id was ${before.auth.fi_tenant_id} (not Evolved) — will fix to ${TENANT_ID}`
    );
  }
