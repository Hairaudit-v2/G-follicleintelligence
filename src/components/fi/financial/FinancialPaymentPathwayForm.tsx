...
export function FinancialPaymentPathwayForm(props: { tenantId: string; canMutate: boolean }) {
  ...
  const [pathwayType, setPathwayType] =
    useState<(typeof PATHWAY_TYPE_OPTIONS)[number]["value"]>("pay_in_full");
  ...
  if (!props.canMutate) {
    return (
      <p className={financialOsClasses.mutedMeta}>
        Finance or manager role required to record payment pathways.
      </p>
    );
  }
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    if (!caseId.trim() && !invoiceId.trim() && !bookingId.trim() && !patientId.trim()) {
      setFeedback({
        message: "Link at least one of patient, case, invoice, or booking id.",
        tone: "warning",
      });
      return;
    }
    ...
    if (cents != null && (!Number.isFinite(cents) || cents < 0)) {
      setFeedback({
        message: "Expected amount (cents) must be a non-negative number.",
        tone: "warning",
      });
      return;
    }
  ...
  return (
    <form
      onSubmit={onSubmit}
      className={`max-w-lg space-y-3 text-xs ${financialOsClasses.formPanel}`}
    >
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <label className={financialOsClasses.formLabel}>
          ...
          <input
            ...
            onChange={(e) => setExpectedAmountCents(e.target.value)}
          />
        </label>
      </div>
      <label className={financialOsClasses.formLabel}>
        Notes
        <textarea
          className={`${financialOsClasses.input} min-h-[4rem]`}
          rows={2}
          value={notes}
