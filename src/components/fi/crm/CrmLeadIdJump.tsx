...
export function CrmLeadIdJump({ tenantId }: { tenantId: string }) {
  ...
  return (
    <form onSubmit={go} className="flex flex-wrap items-end gap-2">
      <div>
        <label htmlFor="crm-lead-id" className="mb-1 block text-xs font-medium text-gray-600">
          Lead UUID
        </label>
        <input
          id="crm-lead-id"
          ...
          className="w-72 max-w-full rounded border border-gray-300 px-2 py-1.5 font-mono text-sm"
        />
      </div>
