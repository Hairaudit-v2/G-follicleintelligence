...
const GALLERY_GROUPS: { id: string; label: string; categories: PatientImageCategory[] }[] = [
  ...
  {
    id: "clinical",
    label: "Clinical captures",
    categories: ["consult", "scalp", "donor", "hairline", "trichoscopy"],
  },
];
    ...
    return (
      <section className={crmLeadCardClass}>
        ...
        <p className="mt-2 text-sm text-slate-400">
          Link a patient via conversion to attach tagged clinical photos.
        </p>
      </section>
    );
  }
        ...
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">
              Images are tagged by category on the patient record. Upload and edit on the patient
              profile.
            </p>
          </div>
          <Link
          ...
          <div className="mt-3">
            {group.tiles.length === 0 ? (
              <p className="text-sm text-gray-500">
                No {group.label.toLowerCase()} images tagged yet.
              </p>
            ) : (
              <PatientImageGrid
                tiles={group.tiles}
      ...
      {selectedTile ? (
        <section className={crmLeadCardClass}>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-300">
            ...
            {selectedTile.image.taken_at ? (
              <span className="text-gray-500">taken {selectedTile.image.taken_at}</span>
            ) : null}
          </div>
        </section>
      ) : null}
