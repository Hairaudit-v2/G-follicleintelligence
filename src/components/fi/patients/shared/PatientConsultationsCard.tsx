  ...
  return (
    <section className={crmLeadCardClass}>
      ...
      {consultations.length === 0 ? (
        ...
        <ul className={`mt-3 divide-y divide-white/[0.06] ${compact ? "max-h-56 overflow-y-auto" : ""}`}>
          {rows.map((c) => (
            <li key={c.id} className="py-2.5">
              ...
              {c.recommendation_notes?.trim() ? (
                <p className="mt-1 line-clamp-2 text-xs text-slate-300">{c.recommendation_notes.trim()}</p>
              ) : null}
            </li>
          ))}
        </ul>
