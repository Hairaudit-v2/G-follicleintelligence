...
export function PatientContactDetailsCard({ data }: { data: PatientProfileFoundationData }) {
  ...
  return (
    <section
      className="rounded border border-white/[0.08] bg-[#0F1629]/80 backdrop-blur-md p-4 shadow-lg shadow-black/40"
      aria-labelledby="patient-contact-details-heading"
    >
      <h2 id="patient-contact-details-heading" className="text-sm font-semibold text-slate-100">
        Contact details
      </h2>
      <p className="mt-1 text-xs text-gray-500">
        Combined from the person record, patient record, and import metadata (for example HubSpot)
        when first-class columns are empty.
      </p>
