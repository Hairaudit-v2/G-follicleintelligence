...
export function PatientCasesCard({
  tenantId,
  data,
}: {
  tenantId: string;
    ...
    <section className="rounded border border-white/[0.08] bg-[#0F1629]/80 backdrop-blur-md p-4 shadow-lg shadow-black/40">
      ...
      {data.cases.length === 0 ? (
        <p className="mt-2 text-sm text-slate-400">
          No clinical patients linked to this foundation patient.
        </p>
      ) : (
        <ul className="mt-3 divide-y divide-white/[0.06]">
          {data.cases.map((c) => (
            <li key={c.id} className="py-2">
              <Link
                href={`/fi-admin/${tenantId}/cases/${c.id}`}
              ...
              {c.sourceLeadId ? (
                <p className="mt-1 text-xs">
                  Source lead:{" "}
                  <Link
                    href={`/fi-admin/${tenantId}/crm/leads/${c.sourceLeadId}`}
