...
function statusLabel(
  status: WorkforceRosterOverview["todaysStaffingReadiness"][number]["status"]
): string {
  switch (status) {
    case "ready":
      return "Ready";
    ...
    <section className="mt-8 rounded-2xl border border-white/[0.08] bg-[#0F1629]/60 p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-400">
            Operational workforce orchestration — shifts, availability, staffing templates, and
            clinical event assignments.
          </p>
        </div>
        <Link
          href={rosterHref}
      ...
      <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-3">
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
          ...
          <dd className="mt-2 text-2xl font-semibold text-slate-100">
            {overview.shiftsScheduledThisWeek}
          </dd>
        </div>
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
          <dt className="flex items-center gap-2 text-slate-500">
          ...
          <dd className="mt-2 text-2xl font-semibold text-slate-100">
            {overview.availabilityBlocksCount}
          </dd>
        </div>
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
          <dt className="flex items-center gap-2 text-slate-500">
          ...
          <dd className="mt-2 text-2xl font-semibold text-slate-100">
            {overview.staffAssignedToEvents}
          </dd>
        </div>
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
          <dt className="flex items-center gap-2 text-slate-500">
          ...
          <dd className="mt-2 text-2xl font-semibold text-amber-300">
            {overview.eventsWithStaffingWarnings}
          </dd>
        </div>
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
          <dt className="flex items-center gap-2 text-slate-500">
          ...
          <dd className="mt-2 text-2xl font-semibold text-rose-300">
            {overview.eventsMissingRequiredRoles}
          </dd>
        </div>
      </dl>
      ...
      <div className="mt-6">
        <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500">
          Today&apos;s staffing readiness
        </h3>
        {overview.todaysStaffingReadiness.length === 0 ? (
          <p className="mt-3 text-sm text-slate-500">
            No clinical event assignments scheduled for today.
          </p>
        ) : (
            ...
            <table className="w-full min-w-[520px] text-left text-sm">
              ...
              <tbody>
                {overview.todaysStaffingReadiness.map((row, idx) => (
                  <tr
                    key={`${row.eventType}-${row.timeLabel}-${idx}`}
                    className="border-b border-white/[0.04]"
                  >
                    <td className="py-2.5 pr-4 capitalize text-slate-200">
                    ...
                    <td className="py-2.5 pr-4 text-slate-400">
                      {formatRequiredRoles(row.requiredRoles)}
                    </td>
                    <td className="py-2.5 pr-4 text-slate-200">{row.assignedStaffCount}</td>
                    <td className={`py-2.5 font-medium ${statusClass(row.status)}`}>
