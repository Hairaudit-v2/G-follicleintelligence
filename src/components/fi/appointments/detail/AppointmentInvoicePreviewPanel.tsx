  ...
  return (
    <section className={appointmentCardClass}>
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <h2 className="text-sm font-semibold text-gray-900">Invoice preview</h2>
      ...
      <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
        <div>
          ...
          <dd className="font-medium text-gray-900">{invoice.reference ?? "—"}</dd>
        </div>
        <div>
          <dt className="text-xs text-gray-500">Status</dt>
          <dd className="capitalize text-gray-900">{invoice.status ?? "draft"}</dd>
        </div>
        <div>
        ...
        <div>
          ...
          <dd className="font-semibold text-gray-900">{invoice.totalLabel ?? "TBC"}</dd>
        </div>
      </dl>
          ...
          <tbody>
            {invoice.lineItems.map((line, i) => (
              <tr key={`${line.label}-${i}`} className="border-b border-gray-100">
                ...
                <td className="py-2 text-right text-gray-800">{line.amount ?? "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ) : (
        <p className="mt-4 text-sm text-gray-600">No line items yet. Add `metadata.invoice` on the booking when billing is wired.</p>
      )}
      {invoice.notes ? <p className="mt-3 text-xs text-gray-600">{invoice.notes}</p> : null}
    </section>
  );
}
