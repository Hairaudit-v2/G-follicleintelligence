  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal>
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl border border-slate-200 bg-white p-5 shadow-xl">
        ...
        <div className="mt-4 flex gap-2 border-b border-slate-100 pb-3 text-xs font-semibold">
          <button
            type="button"
            className={tab === "create" ? "text-sky-700" : "text-slate-500"}
          ...
          <button
            type="button"
            className={tab === "pay" ? "text-sky-700" : "text-slate-500"}
            onClick={() => setTab("pay")}
          ...
          <button
            type="button"
            className={tab === "status" ? "text-sky-700" : "text-slate-500"}
        ...
        {tab === "create" ? (
          <div className="mt-4 space-y-3 text-sm">
            ...
            <label className="block">
              ...
              <input className="mt-1 w-full rounded border border-slate-200 px-2 py-1.5" value={currency} onChange={(e) => setCurrency(e.target.value)} maxLength={3} />
            </label>
            <label className="block">
            ...
            <label className="block">
              ...
              <textarea className="mt-1 w-full rounded border border-slate-200 px-2 py-1.5" rows={3} value={notes} onChange={(e) => setNotes(e.target.value)} />
            </label>
            <button
              type="button"
              disabled={busy}
        ...
        {tab === "pay" ? (
          <div className="mt-4 space-y-3 text-sm">
            <label className="block">
              ...
              <select className="mt-1 w-full rounded border border-slate-200 px-2 py-1.5" value={recordId} onChange={(e) => setRecordId(e.target.value)}>
            ...
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-wide text-slate-500">Notes (optional)</span>
              <textarea className="mt-1 w-full rounded border border-slate-200 px-2 py-1.5" rows={2} value={notes} onChange={(e) => setNotes(e.target.value)} />
            </label>
            <button
              type="button"
              disabled={busy}
        ...
        {tab === "status" ? (
          <div className="mt-4 space-y-3 text-sm">
            ...
            <label className="block">
              ...
              <select className="mt-1 w-full rounded border border-slate-200 px-2 py-1.5" value={status} onChange={(e) => setStatus(e.target.value as PaymentStatus)}>
