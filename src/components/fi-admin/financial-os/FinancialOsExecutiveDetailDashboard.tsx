...
export function FinancialOsExecutiveDetailDashboard(props: {
  ...
  const explanation = Array.isArray(forecastMeta.forecast_explanation)
    ? (forecastMeta.forecast_explanation as Array<{ factor: string; contribution_cents: number; weight: number }>)
    : [];
    ...
    <div className={financialOsClasses.pageShell}>
      <header className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="text-xl font-semibold text-slate-50 sm:text-2xl">Executive finance intelligence</h1>
          <p className="mt-1 text-sm text-slate-400">
            Owner-level revenue, profitability, attribution, AR risk, and forecast — {data.periodStart} to {data.periodEnd}
          </p>
        </div>
        <Link
          href={base}
          className={cn(fiOsChromeClasses.toolbarControlSurface, "px-3 py-1.5 text-xs font-semibold text-slate-200")}
        >
          ← Command centre
        </Link>
      </header>
      ...
      <FinancialOsSectionCard title="Revenue performance" kicker="Ledger + invoices">
        <dl className={financialOsClasses.metricGrid}>
          <FinancialOsMetricTile label="Gross revenue" value={fmtMoney(snapshot.gross_revenue_cents, data.currency)} />
          <FinancialOsMetricTile label="Collected revenue" value={fmtMoney(snapshot.collected_revenue_cents, data.currency)} />
          <FinancialOsMetricTile label="Surgery revenue" value={fmtMoney(snapshot.surgery_revenue_cents, data.currency)} />
          <FinancialOsMetricTile label="Treatment revenue" value={fmtMoney(snapshot.treatment_revenue_cents, data.currency)} />
          <FinancialOsMetricTile label="Paid invoices" value={String(snapshot.total_paid_invoices)} />
      ...
      <FinancialOsSectionCard title="Surgery profitability" kicker="Cost models">
        <dl className={financialOsClasses.metricGrid}>
          <FinancialOsMetricTile label="Gross profit" value={fmtMoney(snapshot.gross_profit_cents, data.currency)} />
          <FinancialOsMetricTile label="Average margin" value={`${snapshot.average_margin_percentage.toFixed(1)}%`} />
          <FinancialOsMetricTile
        ...
        <div className="mt-4">
          ...
          >
            {attribution.revenue_by_source.map((row) => {
              const profit = attribution.profit_by_source.find((p) => p.source === row.source)?.cents ?? 0;
              return (
                <tr key={row.source} className={financialOsClasses.tableRow}>
                  ...
                  <td className={financialOsClasses.tableCellMono}>{fmtMoney(row.cents, data.currency)}</td>
                  <td className={financialOsClasses.tableCellMono}>{fmtMoney(profit, data.currency)}</td>
                </tr>
              );
            })}
          </FinancialOsTable>
      ...
      <FinancialOsSectionCard title="Accounts receivable risk" kicker="Collections">
        <dl className={financialOsClasses.metricGrid}>
          <FinancialOsMetricTile label="Outstanding revenue" value={fmtMoney(snapshot.outstanding_revenue_cents, data.currency)} />
          <FinancialOsMetricTile label="Overdue revenue" value={fmtMoney(snapshot.overdue_revenue_cents, data.currency)} />
          <FinancialOsMetricTile label="Overdue invoices" value={String(snapshot.total_overdue_invoices)} />
          <FinancialOsMetricTile label="AR risk score" value={`${snapshot.ar_risk_score.toFixed(1)} / 100`} />
        </dl>
      </FinancialOsSectionCard>

      <FinancialOsSectionCard title="Forecast" kicker="Deterministic v1">
        <dl className={financialOsClasses.metricGrid}>
          <FinancialOsMetricTile label="Forecast revenue" value={fmtMoney(snapshot.forecast_revenue_cents, data.currency)} />
          <FinancialOsMetricTile label="Confidence" value={`${snapshot.forecast_confidence.toFixed(0)}%`} />
          <FinancialOsMetricTile
        ...
        {explanation.length > 0 ? (
          <div className="mt-4">
            <p className="mb-2 text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Explanation factors</p>
            <FinancialOsTable
              isEmpty={false}
              head={
                <>
            ...
            >
              {explanation.map((f) => (
                <tr key={f.factor} className={financialOsClasses.tableRow}>
                  ...
                  <td className={financialOsClasses.tableCellMono}>{fmtMoney(f.contribution_cents, data.currency)}</td>
      ...
      <FinancialOsSectionCard title="Period comparison" kicker="vs previous">
        <dl className={financialOsClasses.metricGrid}>
          ...
          />
          <FinancialOsMetricTile label="Gross profit delta" value={fmtMoney(comparison.gross_profit_delta_cents, data.currency)} />
          <FinancialOsMetricTile
            label="Margin delta"
            value={`${comparison.margin_delta_percentage_points >= 0 ? "+" : ""}${comparison.margin_delta_percentage_points.toFixed(1)}pp`}
          />
          <FinancialOsMetricTile label="Outstanding delta" value={fmtMoney(comparison.outstanding_delta_cents, data.currency)} />
          <FinancialOsMetricTile
        ...
        {data.previousSnapshot ? (
          <p className="mt-3 text-xs text-slate-500">
            Previous period baseline: {data.previousSnapshot.period_start} → {data.previousSnapshot.period_end} (
          ...
          >
            {data.snapshotHistory.slice(0, 12).map((row) => (
              <tr key={row.id} className={financialOsClasses.tableRow}>
                ...
                <td className={financialOsClasses.tableCellMono}>{fmtMoney(row.collected_revenue_cents, data.currency)}</td>
