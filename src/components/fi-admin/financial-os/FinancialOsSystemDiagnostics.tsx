...
function FinancialOsOperatorAlertStrip({ alerts }: { alerts: FinancialOsCommandCentreAlertStrip }) {
  ...
  const sections = [
    { key: "unmatched", title: "Unmatched payments", items: alerts.unmatchedPayments.items, tone: "critical" as const },
            ...
            <ul className="mt-2 space-y-1.5 text-xs text-slate-300">
              {section.items.slice(0, 8).map((item) => (
                <li key={item.id} className="flex items-start justify-between gap-2">
                  <span>
                    {item.label}
                    <span className="ml-1 font-mono text-[10px] text-slate-500">({item.id.slice(0, 8)}…)</span>
                  </span>
                  <span
  ...
  return (
    ...
    >
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm text-[#94A3B8]">
              For platform operators only. These checks support financial integrity and payment processing without
              affecting day-to-day clinic finance workflow.
            </p>
          </div>
      ...
      <div className="space-y-6 border-t border-white/[0.06] px-5 py-5 sm:px-6 sm:py-6">
        ...
        <DashboardCard className="border-white/[0.06] bg-[#0c1220]/40 p-4 sm:p-5">
          <SectionHeader title="Operational alerts" description="Reconciliation, gateway, and deposit deadline signals." className="mb-3" />
        ...
        <DashboardCard className="border-white/[0.06] bg-[#0c1220]/40 p-4 sm:p-5">
          ...
          <dl className={cn(financialOsClasses.metricGrid, "mt-4")}>
            <div className={financialOsClasses.metricTile}>
              ...
              <dd className={financialOsClasses.metricValue}>{data.surgeryEconomics.metrics.average_margin_percentage.toFixed(1)}%</dd>
            </div>
            <div className={financialOsClasses.metricTile}>
              ...
              <dd className={financialOsClasses.metricValue}>
                {data.surgeryEconomics.metrics.average_revenue_per_graft_cents != null
                  ? fmtFinancialMoney(data.surgeryEconomics.metrics.average_revenue_per_graft_cents, data.surgeryEconomics.currency)
                  : "—"}
              </dd>
            </div>
            <div className={financialOsClasses.metricTile}>
              ...
              <dd className={financialOsClasses.metricValue}>
                {data.surgeryEconomics.metrics.average_cost_per_graft_cents != null
                  ? fmtFinancialMoney(data.surgeryEconomics.metrics.average_cost_per_graft_cents, data.surgeryEconomics.currency)
                  : "—"}
              </dd>
            </div>
          </dl>
            ...
            >
              {data.surgeryEconomics.recentSnapshots.map((row) => (
                <tr key={row.id ?? `${row.case_id}-${row.calculated_at}`} className={financialOsClasses.tableRow}>
                  ...
                  <td className={financialOsClasses.tableCellMono}>{fmtFinancialMoney(row.revenue_cents, data.surgeryEconomics.currency)}</td>
                  <td className={financialOsClasses.tableCellMono}>{fmtFinancialMoney(row.total_cost_cents, data.surgeryEconomics.currency)}</td>
                  <td className={financialOsClasses.tableCellStrong}>{fmtFinancialMoney(row.gross_profit_cents, data.surgeryEconomics.currency)}</td>
                  <td className={financialOsClasses.tableCell}>{row.gross_margin_percentage.toFixed(1)}%</td>
                  <td className={financialOsClasses.tableCellMono}>{row.id?.slice(0, 8) ?? "—"}…</td>
                </tr>
              ))}
            </FinancialOsTable>
          </div>
            ...
            >
              {data.revenueAttribution.rows.map((row) => (
                <tr key={`${row.source}-${row.campaign}`} className={financialOsClasses.tableRow}>
                  ...
                  <td className={financialOsClasses.tableCellMono}>{fmtFinancialMoney(row.collected_revenue_cents, data.revenueAttribution.currency)}</td>
        ...
        <div className={financialOsClasses.dashboardGrid}>
            ...
            >
              {data.recentTransactions.map((tx) => (
                <tr key={tx.id} className={financialOsClasses.tableRow}>
                  <td className={financialOsClasses.tableCellMono}>{fmtFinancialWhen(tx.created_at)}</td>
          ...
          <DashboardCard className="border-white/[0.06] bg-[#0c1220]/40 p-4 sm:p-5">
            <SectionHeader title="Open invoices" description="Sent / awaiting payment / partial / overdue balances." className="mb-3" />
            <FinancialOsTable
              ...
              {data.recentOpenInvoices.map((inv) => (
                <tr key={inv.id} className={financialOsClasses.tableRow}>
                  ...
                  <td className={financialOsClasses.tableCellMono}>
                    {inv.due_date ?? "—"}
                    {inv.days_overdue > 0 ? <span className="ml-1 text-amber-300/90">+{inv.days_overdue}d</span> : null}
                  </td>
                  <td className={financialOsClasses.tableCellStrong}>{fmtFinancialMoney(inv.remaining_balance_cents, inv.currency)}</td>
        ...
        <DashboardCard className="border-white/[0.06] bg-[#0c1220]/40 p-4 sm:p-5">
          <SectionHeader title="Module integrations" description="Cross-OS financial event wiring." className="mb-3" />
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Link href={`${base}/financial/dashboard`} className={cn(fiOsChromeClasses.toolbarControlSurface, "px-3 py-1.5 text-xs font-semibold text-slate-200")}>
              Full FinancialOS module
            </Link>
            <Link href={`${base}/settings/payments`} className={cn(fiOsChromeClasses.toolbarControlSurface, "px-3 py-1.5 text-xs font-semibold text-slate-200")}>
