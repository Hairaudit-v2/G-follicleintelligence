  ...
  return (
    <DashboardCard className="flex h-full min-h-[280px] flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2">
        {surgeries.length === 0 ? (
          <p className="px-2 py-6 text-center text-sm text-slate-500">No surgeries scheduled in the next 14 days.</p>
        ) : (
          <ul className="space-y-1">
            {surgeries.map((s) => {
              ...
              return (
                <li key={s.bookingId}>
                  <div className={cn("rounded-lg px-3 py-2.5", receptionOsSeverityRowClass(s.severity))}>
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        {s.staffAssigned ? <p className="mt-0.5 text-xs text-slate-500">Staff: {s.staffAssigned}</p> : null}
                      </div>
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-1.5">
                      ...
                      {s.readinessPercent != null ? (
                        <span className="text-[0.62rem] text-slate-500">{s.readinessPercent}% ready</span>
                      ) : null}
                    </div>
