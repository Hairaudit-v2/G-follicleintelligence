    ...
    <DashboardCard className="overflow-hidden">
      ...
      <div className="grid gap-4 px-4 py-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <BriefStat
          ...
          value={brief.outstandingDepositCount}
          hint={brief.overdueDepositCount ? `${brief.overdueDepositCount} overdue` : undefined}
        />
        <BriefStat
          label="Surgeries (14d)"
          value={brief.surgeryNext14Count}
          hint={brief.surgeryRiskCount ? `${brief.surgeryRiskCount} at risk` : undefined}
        />
      ...
      <div className="border-t border-white/[0.06] px-4 py-3">
        <div className="mb-2 flex flex-wrap items-center gap-2">
          ...
          <ReceptionOsSeverityBadge
            severity="blocked"
            className={brief.alertsBySeverity.blocked ? undefined : "opacity-40"}
          />
          <span className="text-xs text-slate-500">{brief.alertsBySeverity.blocked}</span>
          <ReceptionOsSeverityBadge
            severity="critical"
            className={brief.alertsBySeverity.critical ? undefined : "opacity-40"}
          />
          <span className="text-xs text-slate-500">{brief.alertsBySeverity.critical}</span>
          <ReceptionOsSeverityBadge
            severity="warning"
            className={brief.alertsBySeverity.warning ? undefined : "opacity-40"}
          />
          <span className="text-xs text-slate-500">{brief.alertsBySeverity.warning}</span>
          <ReceptionOsSeverityBadge
            severity="info"
            className={brief.alertsBySeverity.info ? undefined : "opacity-40"}
          />
