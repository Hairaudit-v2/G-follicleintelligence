...
export function ReceptionOsTodaysPatientsWidget({ patients }: { patients: ReceptionOsTodaysPatient[] }) {
  return (
    <DashboardCard className="flex h-full min-h-[320px] flex-col overflow-hidden">
      <div className="border-b border-white/[0.06] px-4 py-3">
        <SectionHeader title="Today's patients" description={`${patients.length} appointment${patients.length === 1 ? "" : "s"}`} />
      </div>
      <div className="flex-1 overflow-y-auto px-2 py-2">
        {patients.length === 0 ? (
          <p className="px-2 py-6 text-center text-sm text-slate-500">No appointments scheduled for today.</p>
        ) : (
                ...
                <li key={p.id}>
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                      <span className="min-w-[3.5rem] font-mono text-sm tabular-nums text-cyan-300/90">{p.appointmentTime || "—"}</span>
                      <span className="min-w-0 flex-1 truncate font-medium text-slate-100">{p.patientName}</span>
                      <span className="hidden text-xs text-slate-500 sm:inline">{p.appointmentType}</span>
                      <FiStatusBadge tone={statusTone(p.status)}>{p.statusLabel}</FiStatusBadge>
                      <span className="w-full truncate text-xs text-slate-500 sm:w-auto sm:max-w-[8rem]">{p.clinician}</span>
                    </div>
