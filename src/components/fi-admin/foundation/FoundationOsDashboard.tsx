...
function isNewClinicSnapshot(data: FoundationOsDashboardPayload): boolean {
  ...
  return (
    t.fi_cases === 0 && t.fi_patients === 0 && t.fi_persons === 0 && t.fi_timeline_events === 0
  );
}
...
function MonoLink({ href, children }: { href: string; children: ReactNode }) {
  return (
    <Link
      href={href}
      className="font-mono text-xs text-cyan-300/90 hover:text-cyan-200 hover:underline"
    >
      {children}
    </Link>
  );
...
export function FoundationOsDashboard({
  ...
  const unifiedRecords =
    id.resolution_rows_with_foundation > 0
      ? id.resolution_rows_with_foundation
      : th.foundation_patients;
  const surgeryLinkedPatientsEstimate =
    th.foundation_patients > 0
      ? Math.round((tc.surgeryos_linkage_pct / 100) * th.foundation_patients)
      : 0;
  const missingMediaPatients = Math.max(
    0,
    th.foundation_patients - th.patients_with_unified_media_distinct
  );
  ...
  return (
    <div className="min-w-0 space-y-6 pb-10">
      ...
      <TwinSectionCard
        id="twin-health"
        title="Clinical twin health"
        description="How complete and navigable patient records are across FI OS."
      >
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
          <SummaryTile
            label="Total patients"
            value={th.foundation_patients}
            sub="All patient records in this clinic"
          />
          <SummaryTile
          ...
          <SummaryTile
            ...
            sub={
              coveragePercentLabel(tc.media_coverage_pct) !== "—"
                ? `${coveragePercentLabel(tc.media_coverage_pct)} of patients`
                : "Clinical photography linked"
            }
            tone="info"
          />
          <SummaryTile
          ...
          <SummaryTile
            ...
            sub={
              th.reports_total > 0
                ? `${th.reports_total} report${th.reports_total === 1 ? "" : "s"} in pipeline`
                : "No reports yet"
            }
          ...
          <SummaryTile
            ...
            tone={
              tc.twin_readiness_score_hint >= 70
                ? "info"
          ...
          {patientOs.recentPatients.length === 0 ? (
            ...
            <ul className="overflow-hidden rounded-xl border border-white/[0.06] bg-[#0a101f]/50">
              {patientOs.recentPatients.map((p) => {
                ...
                const displayName =
                  p.displayName !== "—" ? p.displayName : (p.phone ?? p.email ?? "Unnamed patient");
                ...
                return (
                  <TwinListRow
                    ...
                    primary={
                      <Link
                        href={`${base}/patients/${p.patientId}/twin`}
                    ...
                    actions={
                      <>
                        ...
                        {journey
                          ? actionLink(`${base}/cases/${journey.caseId}`, "Open case")
                          : null}
            ...
            <ul className="overflow-hidden rounded-xl border border-white/[0.06] bg-[#0a101f]/50">
              {patientOs.activeJourneys.map((j) => (
                <TwinListRow
                  ...
                  primary={
                    <Link
                      href={`${base}/patients/${j.patientId}/twin`}
                      className="hover:text-cyan-200 hover:underline"
                    >
              ...
              <TwinListRow
                ...
                primary={
                  <span className="text-slate-100">
                    {h.title ?? h.eventKind.replace(/_/g, " ")}
                  </span>
                }
                meta={
                  <>
                    ...
                    {h.patientId ? (
                      <>
                        ...
                        <Link
                          ...
                          className="text-cyan-300/90 hover:text-cyan-200 hover:underline"
                        >
                ...
                actions={
                  <>
                    {h.patientId
                      ? actionLink(
                          `${base}/patients/${h.patientId}/timeline`,
                          "View timeline",
                          true
                        )
                      : null}
        ...
        >
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <SummaryTile
              ...
              sub={
                coveragePercentLabel(tc.media_coverage_pct) !== "—"
                  ? `${coveragePercentLabel(tc.media_coverage_pct)} coverage`
                  : undefined
              }
              tone="info"
            />
            ...
            <SummaryTile
              ...
              sub={
                photoProtocol
                  ? "Protocol sessions not yet complete"
                  : "Photo protocol analytics unavailable"
              }
              tone={
      ...
      <details className={patientTwinSectionClass} open={showDiagnosticsExpanded}>
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm leading-relaxed text-slate-500">
                For platform operators only. These checks support data integrity and do not affect
                day-to-day clinical workflows.
              </p>
            </div>
            <span className="shrink-0 text-xs font-medium text-cyan-400/80">
        ...
        <div className="mt-5 space-y-5 border-t border-white/[0.06] pt-5">
          <InfoNotice variant="warning" title="Read-only observability">
            Aggregates tenant-scoped foundation tables and compatibility views. Does not run
            identity merges, alter RLS, or change ingestion. Optional backfill below is a manual
            operator tool — not automatic repair.
          </InfoNotice>
          ...
          <div className="grid min-w-0 grid-cols-1 gap-5 lg:grid-cols-2">
            <DashboardCard className="p-4 sm:p-5">
              <SectionHeader
                ...
                description="v_fi_patient_resolution and duplicate heuristics."
              />
              <ul className="mt-3 space-y-2 text-sm text-slate-400">
                <li>
                  <span className="text-slate-300">Resolution rows with foundation patient:</span>{" "}
                  {id.resolution_rows_with_foundation}
                </li>
                <li>
                  <span className="text-slate-300">Global-only (unresolved) rows:</span>{" "}
                  {id.resolution_rows_global_only}
                </li>
                <li>
                  <span className="text-slate-300">Foundation-only (no global stub):</span>{" "}
                  {id.resolution_rows_foundation_only_no_global}
                </li>
                <li>
                  <span className="text-slate-300">Duplicate-risk person emails (groups):</span>{" "}
                  {id.duplicate_person_email_groups}
                </li>
                <li>
                  <span className="text-slate-300">Duplicate patient rows / person_id:</span>{" "}
                  {id.duplicate_patient_rows_same_person_id}
                </li>
              </ul>
            </DashboardCard>
            ...
            <DashboardCard id="fi-os-foundation-media" className="scroll-mt-24 p-4 sm:p-5">
              <SectionHeader
                title="Media health (technical)"
                description="Legacy uploads, foundation assets, and unified view."
              />
              <ul className="mt-3 space-y-2 text-sm text-slate-400">
                <li>
                ...
                <li>
                  <span className="text-slate-300">Unified media rows:</span>{" "}
                  {med.unified_media_rows === null ? "—" : med.unified_media_rows}
                </li>
                <li>
                  <span className="text-slate-300">Unified rows without case:</span>{" "}
                  {med.unified_media_without_case}
                </li>
                <li>
                  <span className="text-slate-300">fi_media_assets without case_id:</span>{" "}
                  {med.fi_media_assets_without_case_id}
                </li>
                <li>
                  <span className="text-slate-300">Unified rows without patient:</span>{" "}
                  {med.unified_rows_without_patient === null
                    ? "—"
                    : med.unified_rows_without_patient}
                </li>
              </ul>
              {med.fi_uploads > 0 && med.fi_media_assets === 0 ? (
                <p className="mt-3 rounded-lg border border-amber-500/20 bg-amber-950/25 p-2 text-xs text-amber-100/90">
                  fi_uploads has rows but fi_media_assets is empty — confirm foundation media
                  dual-write for this tenant.
                </p>
              ) : null}
            </DashboardCard>
          </div>
          ...
          <DashboardCard className="p-4 sm:p-5">
            <SectionHeader
              ...
              description="Raw ingest vs curated timeline and event linkage chain."
            />
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <SummaryTile label="fi_events" value={te.fi_events} sub="All statuses" />
              <SummaryTile
                label="Processed fi_events"
                value={te.fi_events_processed}
                sub="status = processed"
                tone="info"
              />
              <SummaryTile
                label="Events (last 7 days)"
                value={te.fi_events_last_7_days}
                sub="created_at window"
                tone="info"
              />
              <SummaryTile
                label="fi_timeline_events"
                value={te.fi_timeline_events}
                sub="Curated timeline"
              />
              <SummaryTile
                label="Timeline w/ patient_id"
                value={te.timeline_events_with_patient_id}
                sub="Rows with patient anchor"
                tone="info"
              />
              <SummaryTile
              ...
              />
              <SummaryTile
                label="Events w/ case link"
                value={te.events_with_fi_case_link}
                sub="Latest link has fi_case_id"
                tone="info"
              />
              <SummaryTile
          ...
          <DashboardCard className="p-4 sm:p-5">
            ...
            <div className="mt-4 grid min-w-0 grid-cols-1 gap-6 lg:grid-cols-2">
              <div>
                <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                  Global patients without foundation link
                </h3>
                  ...
                  {m.previews.unresolved_global_patients.length === 0 ? (
                    ...
                    m.previews.unresolved_global_patients.map((r) => (
                      <li key={r.global_patient_id} className="px-3 py-2 text-slate-400">
                        <MonoLink href={`${base}/patients/${r.global_patient_id}`}>
                          {r.global_patient_id}
                        </MonoLink>
              ...
              <div>
                <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                  Cases missing foundation patient (preview)
                </h3>
                <ul className="mt-2 max-h-48 divide-y divide-white/[0.06] overflow-auto rounded-lg border border-white/[0.06] bg-[#0a101f]/50 text-sm">
            ...
            <div className="mt-4">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                Duplicate-risk: shared email_normalized
              </h3>
            ...
            <p className="mt-3 text-xs text-slate-400">
              v_fi_case_foundation rows missing foundation patient:{" "}
              {m.risks.unresolved_cases_no_foundation_patient}. Media without case (unified view):{" "}
              {m.unified_media_without_case_id}.
            </p>
          </DashboardCard>

          {photoProtocol && photoProtocol.alert_events.length > 0 ? (
            <PhotoProtocolAlertEventsTable
              tenantId={tid}
              events={photoProtocol.alert_events}
              variant="darkGlass"
            />
          ) : null}

          {photoProtocol && photoProtocol.incomplete_sessions.length > 0 ? (
            <PhotoProtocolIncompleteSessionsTable
              tenantId={tid}
              rows={photoProtocol.incomplete_sessions}
              variant="darkGlass"
            />
          ) : null}
