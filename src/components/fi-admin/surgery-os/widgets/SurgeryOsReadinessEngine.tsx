...
export function SurgeryOsReadinessEngineWidget({ snapshots }: { snapshots: SurgeryOsReadinessSnapshot[] }) {
  return (
    <DashboardCard className="flex h-full min-h-[320px] flex-col overflow-hidden">
      <div className="border-b border-white/[0.06] px-4 py-3">
        <SectionHeader title="Surgical readiness engine" description="Pre-op checklist · risk scoring" />
      </div>
      <div className="flex-1 overflow-y-auto px-2 py-2">
        {snapshots.length === 0 ? (
          ...
          <ul className="space-y-2">
            {snapshots.map((snap) => {
              const riskStyles = SURGERY_OS_RISK_SURFACE[snap.readinessRiskLevel] ?? SURGERY_OS_RISK_SURFACE.medium;
              const href = snap.hrefs.case ?? snap.hrefs.patient ?? snap.hrefs.surgery;
              const inner = (
                <div className={cn("rounded-lg border px-3 py-3", riskStyles.border, riskStyles.bg)}>
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn("text-xs font-semibold", riskStyles.text)}>{snap.readinessRiskLabel}</span>
                    </div>
                  </div>
                  ...
                  <ul className="mt-3 grid gap-1 sm:grid-cols-2">
                    {snap.checklist.map((item) => (
                      <li key={item.key} className="flex items-center gap-1.5 text-xs text-slate-400">
                        {item.complete ? (
