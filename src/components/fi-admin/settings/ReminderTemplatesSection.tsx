...
export function ReminderTemplatesSection(props: {
  ...
  const [trigger, setTrigger] =
    useState<(typeof REMINDER_TRIGGER_EVENTS)[number]>("booking_24h_before");
      ...
      <div className={sectionClass}>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <label className="block text-xs text-[#94A3B8]">
            ...
            <select
              className={`${inputClass} mt-1`}
              value={type}
              onChange={(e) => setType(e.target.value as (typeof REMINDER_TEMPLATE_TYPES)[number])}
            >
          ...
          <button
            ...
            onClick={() => {
              ...
              startTransition(async () => {
                const r = await previewReminderTemplateAction({
                  body,
        ...
        {preview ? (
          <div className="mt-4 rounded-lg border border-white/[0.08] bg-[#081020]/60 p-3 text-xs text-[#CBD5E1]">
            ...
            {preview.subject ? (
              <p className="mt-1 text-[#F8FAFC]">Subject: {preview.subject}</p>
            ) : null}
                ...
                {sorted.map((row) => (
                  <tr key={row.id} className="border-t border-white/[0.06]">
                    ...
                    <td className="py-2 align-top">
                      <button
                        type="button"
                        className="mr-2 text-[#22C1FF] hover:underline"
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (
                            !window.confirm(
                              "Delete this template? Pending jobs for this template will be removed by FK cascade."
                            )
                          )
                            return;
                          startTransition(async () => {
                            const r = await deleteReminderTemplateAction(
                              tenantId,
                              row.id,
