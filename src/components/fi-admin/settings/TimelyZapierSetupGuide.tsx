    ...
    <div className="space-y-8">
      <section className="rounded-xl border border-white/[0.08] bg-[#0a1424]/80 p-4 sm:p-5">
        ...
        <p className="mt-1 text-sm text-[#94A3B8]">
          Deployment checks and tenant-scoped Timely sync stats (read-only). Webhook secret is never
          shown.
        </p>
              ...
              setup.lastAppointment ? (
                <span className="text-right">
                  ...
                  <span className="mt-0.5 block text-xs text-[#94A3B8]">
                    mapping {setup.lastAppointment.created_at} · external{" "}
                    {setup.lastAppointment.external_id}
                  </span>
                </span>
              ) : (
                "—"
      ...
      <section className="rounded-xl border border-white/[0.08] bg-[#0a1424]/80 p-4 sm:p-5">
        ...
        <p className="mt-1 text-sm text-[#94A3B8]">
          Use your current browser host as <span className="text-[#CBD5E1]">APP_URL</span> below, or
          set{" "}
          <code className="rounded bg-[#141C33] px-1.5 py-0.5 text-xs text-[#22C1FF]">
            NEXT_PUBLIC_SITE_URL
          </code>{" "}
          for stable production links.
        </p>
      ...
      <section className="rounded-xl border border-white/[0.08] bg-[#0a1424]/80 p-4 sm:p-5">
        ...
        <p className="mt-1 text-sm text-[#94A3B8]">
          Last 20 Timely appointment mappings for this tenant.
        </p>
            ...
            <table className="w-full min-w-[640px] border-collapse text-left text-sm">
              ...
              <tbody>
                {setup.recentBookingImports.map((row) => (
                  <tr
                    key={`${row.external_id}-${row.created_at}`}
                    className="border-b border-white/[0.05] last:border-b-0"
                  >
                    <td className="px-3 py-2 font-mono text-xs text-[#94A3B8]">{row.created_at}</td>
                    <td className="px-3 py-2 font-mono text-xs text-[#CBD5E1]">
                      {row.external_id}
                    </td>
