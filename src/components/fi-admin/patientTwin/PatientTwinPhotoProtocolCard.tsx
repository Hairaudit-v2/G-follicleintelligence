...
export function PatientTwinPhotoProtocolCard({
  ...
  if (!pp) {
    return (
      <section
        id="smart-photo-protocol"
        className="rounded-lg border border-white/[0.08] bg-[#0b1220]/80 p-4 text-sm text-slate-400"
      >
        <h2 className="text-sm font-semibold text-white">Smart Photography Protocol</h2>
        <p className="mt-2">
          Protocol templates are not available yet (database migration or seed missing).
        </p>
      </section>
    );
  }
  ...
  const pct = pp.compliance.required_count
    ? Math.round((100 * done) / pp.compliance.required_count)
    : 0;

  return (
    <section
      id="smart-photo-protocol"
      className="rounded-lg border border-white/[0.08] bg-[#0b1220]/80 p-4"
    >
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 className="text-sm font-semibold uppercase tracking-wide text-cyan-400/90">
            Smart Photography Protocol
          </h2>
        ...
        <div className="text-right text-xs text-slate-400">
          {pp.active_session_id ? (
            <span>
              Session: {pp.active_session_id.slice(0, 8)}… ({pp.active_session_status})
            </span>
          ) : (
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        <button
          ...
          onClick={() => {
            setMsg(null);
            start(async () => {
              const res = await createPhotoProtocolSessionAction(tenantId, patientId, {
                clinical_context: ctx as (typeof CTX_OPTS)[number],
              });
        ...
        {pp.active_session_id ? (
          <button
            ...
            title={
              !pp.can_complete_session
                ? "Complete required checklist (accept or strong capture) first."
                : undefined
            }
            onClick={() => {
              setMsg(null);
              start(async () => {
                const res = await completePhotoProtocolSessionAction(
                  tenantId,
                  patientId,
                  pp.active_session_id!,
                  {}
      ...
      <div className="mt-4 space-y-3">
        ...
        <ul className="space-y-2">
          {pp.checklist.map((row) => {
            ...
            const thumb = row.patient_image_id
              ? galleryById.get(row.patient_image_id)
              : sug
                ? galleryById.get(sug.image_id)
                : null;
            return (
              <li
                key={row.slot_id + row.session_slot_id}
                className="rounded-md border border-white/[0.06] bg-white/[0.03] p-2 text-xs"
              >
                <div className="flex flex-wrap items-start gap-2">
                  {thumb ? (
                    ...
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded bg-white/5 text-[10px] text-slate-500">
                      —
                    </div>
                  )}
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-slate-400">
                      ...
                      {row.ai_match_confidence != null
                        ? ` · match ${row.ai_match_confidence.toFixed(2)}`
                        : null}
                    </p>
                    {row.capture_guidance ? (
                      <p className="mt-1 text-slate-500">{row.capture_guidance}</p>
                    ) : null}
                  </div>
                </div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {sug ? (
                      <button
                        ...
                        onClick={() => {
                          setMsg(null);
                          start(async () => {
                            const res = await attachImageToProtocolSlotAction(
                              tenantId,
                              patientId,
                              pp.active_session_id!,
                              {
                    ...
                    <button
                      ...
                      onClick={() => {
                        setMsg(null);
                        start(async () => {
                          const res = await acceptProtocolSlotAction(
                            tenantId,
                            patientId,
                            pp.active_session_id!,
                            {
                              session_slot_row_id: row.session_slot_id,
                              ...(row.patient_image_id
                                ? { patient_image_id: row.patient_image_id }
                                : {}),
                            }
                          );
                          if (!res.ok) setMsg(res.error);
                      ...
                      onClick={() => {
                        const note =
                          typeof window !== "undefined"
                            ? window.prompt("Retake reason?", "Needs retake")
                            : null;
                        if (!note?.trim()) return;
                        setMsg(null);
                        start(async () => {
                          const res = await markProtocolSlotNeedsRetakeAction(
                            tenantId,
                            patientId,
                            pp.active_session_id!,
                            {
