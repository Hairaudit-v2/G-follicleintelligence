...
export function PatientTwinOutcomeJourneyCard(props: {
  ...
  return (
    ...
    >
      <h2
        id="twin-outcome-journey-heading"
        className="text-sm font-semibold tracking-tight text-slate-100"
      >
        Outcome Journey
      </h2>
      <p className="mt-1 text-xs text-slate-400">
        Checkpoints, measurements, imaging and audit references, and protocol timeline
        (tenant-scoped). No clinical advice or predictions.
      </p>
            ...
            <ul className="mt-2 space-y-2 text-sm text-slate-200">
              {measurements.map((m) => (
                ...
                >
                  ...
                  {m.measurement_date ? (
                    <span className="text-slate-400"> · {m.measurement_date}</span>
                  ) : null}
                  {m.case_id ? (
                    <span className="block text-xs text-slate-500">Case-linked</span>
                  ) : null}
                </li>
              ))}
            </ul>
