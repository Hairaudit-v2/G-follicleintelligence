...
export function PatientTwinMedicationsCard({ twin }: { twin: PatientTwinV1 }) {
  ...
  return (
    <section className={card}>
      <h2 className="text-sm font-semibold text-slate-100">MedicationOS — therapy</h2>
      ...
      {!hasSignal ? (
        <p className="mt-3 text-sm text-slate-400">No active MedicationOS therapy plans or events for this patient yet.</p>
      ) : (
        <>
              ...
              <table className="min-w-full text-left text-xs text-slate-200">
                ...
                <tbody>
                  {m.active_items.map((row) => (
                    <tr key={row.plan_item_id} className="border-b border-white/[0.06] last:border-0">
                      <td className="py-2 pr-3 align-top">
                        <div className="font-medium text-slate-100">{row.plan_title}</div>
                        <div className="text-gray-500">{row.plan_type}</div>
                      </td>
                      <td className="py-2 pr-3 align-top">
                        <div>{row.display_name}</div>
                        <div className="font-mono text-[10px] text-gray-500">{row.canonical_code}</div>
                        {row.dosing_summary ? <div className="mt-0.5 text-slate-400">{row.dosing_summary}</div> : null}
                      </td>
                      <td className="py-2 pr-3 align-top text-slate-300">{row.role}</td>
                      <td className="py-2 pr-3 align-top">
                        {row.prescription_id ? (
                          <span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-emerald-300">Linked Rx</span>
                        ) : (
          ...
          {m.therapy_events_preview.length > 0 ? (
            <div className="mt-5">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-300">Recent therapy events</h3>
              <ul className="mt-2 space-y-2">
                {m.therapy_events_preview.map((ev) => (
                  <li key={ev.id} className="rounded border border-white/[0.06] bg-white/[0.03] px-3 py-2 text-xs text-slate-200">
                    ...
                    <div className="mt-1 flex flex-wrap gap-2 text-[11px] text-slate-400">
                      ...
                      <span className="rounded bg-[#0F1629]/80 backdrop-blur-md px-1.5 py-0.5 text-gray-500">{ev.source_table}</span>
                    </div>
                  </li>
                ))}
              </ul>
