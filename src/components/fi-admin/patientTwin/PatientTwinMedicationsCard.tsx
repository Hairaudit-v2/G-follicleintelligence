  ...
  return (
    <section className={card}>
      ...
      <p className="mt-1 text-xs text-slate-400">
        Read-only active plans and recent therapy events (caps: {m.active_item_cap} line items,{" "}
        {m.therapy_events_preview_cap} events). Loader tables:{" "}
      ...
      {!hasSignal ? (
        <p className="mt-3 text-sm text-slate-400">
          No active MedicationOS therapy plans or events for this patient yet.
        </p>
      ) : (
        <>
                  ...
                  {m.active_items.map((row) => (
                    ...
                    >
                      ...
                      <td className="py-2 pr-3 align-top">
                        ...
                        {row.dosing_summary ? (
                          <div className="mt-0.5 text-slate-400">{row.dosing_summary}</div>
                        ) : null}
                      </td>
          ...
          {m.therapy_events_preview.length > 0 ? (
            <div className="mt-5">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-300">
                Recent therapy events
              </h3>
              <ul className="mt-2 space-y-2">
                {m.therapy_events_preview.map((ev) => (
                  <li
                    key={ev.id}
                    className="rounded border border-white/[0.06] bg-white/[0.03] px-3 py-2 text-xs text-slate-200"
                  >
                    ...
                    <div className="mt-1 flex flex-wrap gap-2 text-[11px] text-slate-400">
                      ...
                      <span className="rounded bg-[#0F1629]/80 backdrop-blur-md px-1.5 py-0.5 text-gray-500">
                        {ev.source_table}
                      </span>
                    </div>
                  </li>
