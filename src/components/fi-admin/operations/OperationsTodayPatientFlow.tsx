...
const BUCKET_META: { key: keyof ReturnType<typeof countAgendaBookingsOnOperationalDayByBucket>; label: string; icon: typeof Stethoscope }[] = [
...
export function OperationsTodayPatientFlow(props: {
  ...
  const counts = countAgendaBookingsOnOperationalDayByBucket(data.agendaByBucket, todayYmd, calendarTimezone);
        ...
        {BUCKET_META.map(({ key, label, icon: Icon }) => (
          ...
          >
            <div className="flex items-center justify-between gap-2">
              <p className="text-[0.62rem] font-semibold uppercase tracking-[0.16em] text-slate-500">{label}</p>
              <Icon className="h-4 w-4 shrink-0 text-cyan-400/90" aria-hidden />
            </div>
            <p className="mt-2 font-mono text-xl font-semibold tabular-nums text-slate-50">{counts[key]}</p>
          </Link>
        ))}
      </div>
      <p className="mt-3 text-xs leading-relaxed text-slate-500">
        Total today in window: <span className="font-mono text-slate-300">{total}</span> · Open{" "}
        <Link className="text-cyan-400/90 underline-offset-2 hover:underline" href={`${base}/calendar`}>
          calendar
        </Link>{" "}
        for detail.
      </p>
