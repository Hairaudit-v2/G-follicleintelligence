  ...
  return (
    <div className="space-y-3 rounded-xl border border-white/[0.08] bg-white/[0.03] p-4">
      <div>
        ...
        <p
          className={cn(
            "mt-2 rounded-lg border border-violet-500/20 bg-violet-500/10 px-3 py-2 text-xs text-violet-200"
          )}
        >
          Draft note is assembled from structured answers in this consultation (deterministic — not
          a live cloud model). Approve to copy into the chart note, or edit freely.
        </p>
      </div>
        ...
        <>
          <div>
            ...
            >
              {draft.trim()
                ? draft
                : "— Add intake, pattern, and recommendation fields to populate this draft."}
            </div>
          </div>
          ...
          {normalized.note.trim() ? (
            <p className={cn("text-xs", fiOsLightFormSurfaceClassNames.meta)}>
              ...
              {normalized.note.trim() === draft.trim()
                ? "Matches current suggestion."
                : "Differs from current suggestion — approve again to overwrite."}
            </p>
          ) : required ? (
            <p
              className={cn(
