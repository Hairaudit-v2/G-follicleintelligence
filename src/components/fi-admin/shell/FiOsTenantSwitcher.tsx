...
export function FiOsTenantSwitcher({
  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      >
        <DropdownMenuLabel className="px-2 py-1.5 text-xs font-normal text-[#64748B]">
          Clinic / tenant
        </DropdownMenuLabel>
        ...
        {tenants?.map((t) => {
          ...
          return (
            <DropdownMenuItem
              key={t.id}
              asChild
              className="cursor-pointer rounded-lg px-2 py-2 focus:bg-white/[0.06]"
            >
              <Link href={`/fi-admin/${t.id}`} className="flex min-w-0 items-center gap-2">
                ...
                {active ? (
                  <Check className="h-4 w-4 shrink-0 text-[#22C1FF]" aria-label="Current" />
                ) : null}
              </Link>
            </DropdownMenuItem>
          );
