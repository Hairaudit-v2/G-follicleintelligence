  ...
  return (
    <div className="space-y-4">
      {readiness.depositReadinessMessage ? (
        <p className="rounded border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-950">{readiness.depositReadinessMessage}</p>
      ) : null}
      {err ? <p className="text-sm font-medium text-red-700">{err}</p> : null}
      {banner ? <p className="text-xs font-medium text-emerald-700">{banner}</p> : null}

      {canMutate && open.length > 0 ? (
        <div className="flex flex-wrap gap-2">
          <button
            type="button"
            className="rounded border border-blue-200 bg-blue-50 px-2 py-1 text-xs font-semibold text-blue-900 hover:bg-blue-100 disabled:opacity-50"
      ...
      {!canMutate ? (
        <p className="text-sm text-slate-600">Finance or manager access is required to create payment links and invoices.</p>
      ) : (
        <div className="rounded border border-slate-200 bg-slate-50/80 p-3 text-sm text-slate-800">
          <p className="font-medium text-slate-900">Create case invoices</p>
          <div className="mt-2 grid gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              <p className="text-xs text-slate-600">Surgery deposit (when no deposit invoice exists)</p>
              <input
                type="date"
            ...
            <div className="space-y-1">
              <p className="text-xs text-slate-600">Surgery balance</p>
              <input
                type="date"
                className="w-full rounded border border-slate-300 px-2 py-1 text-xs"
                value={balanceDue}
      ...
      {open.length === 0 ? (
        <p className="text-sm text-slate-600">No open invoices with a balance for this case.</p>
      ) : (
      ...
      {patientFoundationId ? (
        <p className="text-xs text-slate-600">
          <Link href={`/fi-admin/${tenantId}/patients/${patientFoundationId}?tab=payments`} className="font-medium text-blue-600 hover:underline">
  ...
  return (
    <li className="space-y-2 px-3 py-3 text-sm">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <p className="font-medium text-slate-900">{inv.title ?? inv.invoice_kind}</p>
          <p className="text-xs text-slate-600">
        ...
        <div className="grid gap-2 border-t border-slate-100 pt-2 sm:grid-cols-2">
          <label className="block text-xs text-slate-600">
            ...
            <div className="mt-1 flex gap-1">
              <input type="date" className="w-full rounded border border-slate-300 px-2 py-1 text-xs" value={due} onChange={(e) => setDue(e.target.value)} />
              <button
                type="button"
                className="shrink-0 rounded border border-slate-300 px-2 text-xs font-medium text-slate-800 hover:bg-slate-100 disabled:opacity-50"
          ...
          <label className="block text-xs text-slate-600 sm:col-span-2">
            ...
            <textarea className="mt-1 w-full rounded border border-slate-300 px-2 py-1 text-xs" rows={2} value={note} onChange={(e) => setNote(e.target.value)} />
          </label>
          <label className="block text-xs text-slate-600 sm:col-span-2">
          ...
          <div className="flex flex-wrap gap-2 sm:col-span-2">
            ...
            <button
              type="button"
              className="rounded border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-800 hover:bg-slate-50 disabled:opacity-50"
              disabled={Boolean(busy)}
              onClick={async () => {
                const cents = audToCents(aud);
                if (!cents) {
