      ...
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-xs text-gray-500">
            Stage 5D: recovery notes, follow-ups, complications, satisfaction, and qualitative
            outcomes. This is not HairAudit scoring, formal surgical audit grading, automated graft
            survival, AI outcome scoring, or certification scoring.
          </p>
        </div>
        {tracking ? (
          <p className="text-xs text-slate-400">
            ...
            <span className="font-medium text-slate-100">
              {postOpStatusLabel(tracking.post_op_status)}
            </span>
            <span className="ml-2 text-gray-400">
              · updated {tracking.updated_at ? tracking.updated_at.slice(0, 10) : "—"}
            </span>
          </p>
        ) : (
          <p className="text-xs text-amber-300">
            No post-op tracking row yet — save the form below to create one.
          </p>
        )}
      </div>
