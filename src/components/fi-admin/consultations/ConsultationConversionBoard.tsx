...
import type {
  ConsultationConversionBoardCard,
  ConsultationConversionBoardPayload,
} from "@/src/lib/consultations/consultationConversionBoardLoader.server";
...
const COLUMN_META: { id: ConsultationConversionBoardColumnId; title: string; tone: string }[] = [
  ...
  },
  {
    ...
    tone: "border-indigo-500/25 bg-indigo-500/[0.06]",
  },
  {
    id: "quote_drafted",
    title: "Quote drafted",
...
export function ConsultationConversionBoard({
  tenantId,
  data,
}: {
  tenantId: string;
  data: ConsultationConversionBoardPayload;
}) {
  const base = `/fi-admin/${tenantId}`;
  const { kpis, window, columns } = data;

  const ratePct =
    kpis.conversionRateQuoteToSurgery != null
      ? `${Math.round(kpis.conversionRateQuoteToSurgery * 1000) / 10}%`
      : "—";
    ...
    <div className="mx-auto max-w-[1920px] space-y-6 pb-10">
      ...
      <DashboardCard className="p-4 sm:p-5">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          <KpiTile
            label="Consults booked (next 30d)"
            value={kpis.consultationsBookedNext30Days}
            highlight
          />
          <KpiTile
            label="Consults completed (last 30d)"
            value={kpis.consultationsCompletedLast30Days}
          />
        ...
        <p className="mt-3 text-[0.65rem] text-slate-500">
          Deposit labels reflect manual payment tracking rows only — no row means &quot;No manual
          deposit record yet.&quot;, not unpaid.
        </p>
      </DashboardCard>
        ...
        <div className="flex min-w-[1200px] gap-3 lg:min-w-0 lg:grid lg:grid-cols-7 lg:gap-3">
          {COLUMN_META.map((col) => (
            <section
              ...
              className={cn(
                "flex w-[280px] shrink-0 flex-col rounded-xl border p-3 lg:w-auto lg:min-w-0",
                col.tone
              )}
            >
              <h2
              ...
              <div className="mt-3 flex max-h-[70vh] min-h-[120px] flex-col gap-2 overflow-y-auto pr-0.5">
                {columns[col.id].length === 0 ? (
                  <p className="px-1 py-6 text-center text-xs leading-relaxed text-slate-400">
                    No consultations in this stage for the current window. Adjust CRM stages,
                    bookings, or filters elsewhere if this looks wrong.
                  </p>
                ) : (
