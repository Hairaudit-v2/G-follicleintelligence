...
export function buildPatientTwinWorkspaceInsights(
  ...
  if (identityPct === null || th.foundation_patients === 0) {
    insights.push("Patient records will appear here as the clinic onboard patients and link identities.");
    ...
    insights.push("Identity unification is still in progress — prioritise linking new enquiries to patient records.");
  }

  const mediaPct = tc.media_coverage_pct;
  if (mediaPct !== null) {
    if (mediaPct >= 70) {
      insights.push("Media coverage is strong — most patient records include linked clinical photography.");
    } else if (mediaPct >= 35) {
      insights.push("Media coverage is partial; schedule photo capture for patients without linked imaging.");
    } else if (th.foundation_patients > 0) {
      insights.push("Media coverage is incomplete for many records; photo capture should be prioritised.");
    }
  }
  ...
  if (auditPct === null) {
    if (th.reports_total === 0 && th.foundation_patients > 0) {
      insights.push("Audit linkage is still limited; outcomes will strengthen as more reports are released.");
    }
  ...
  } else if (tc.twin_readiness_score_hint > 0 && tc.twin_readiness_score_hint < 45 && th.foundation_patients > 0) {
    insights.push("Clinical twin readiness is early — focus on media capture, case linking, and timeline milestones.");
  }
