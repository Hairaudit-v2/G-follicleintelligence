...
export function isFiIntelligenceGovernedDispatchEnabled(options?: GovernedReplayEnvOptions): boolean {
  const env = options?.env ?? (typeof process !== "undefined" ? (process.env as Record<string, string | undefined>) : {});
 ...
 */
export function isDispatchFutureExecutionPolicyAllowed(options?: GovernedReplayEnvOptions): boolean {
  const ne = nodeEnv(options);
  if (ne === "production") return false;
  return isFiIntelligenceGovernedReplayEnabled(options) && isFiIntelligenceGovernedDispatchEnabled(options);
}
