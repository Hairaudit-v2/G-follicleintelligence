  ...
  switch (country) {
    case "AU": {
      ...
      return {
        ...
        gstRatePercent:
          typeof o.gstRatePercent === "number" && Number.isFinite(o.gstRatePercent)
            ? o.gstRatePercent
            : b.gstRatePercent,
      };
    }
    case "IN": {
      ...
      const gh =
        o.gstHandling === "igst" || o.gstHandling === "mixed" || o.gstHandling === "cgst_sgst"
          ? o.gstHandling
          : b.gstHandling;
      return {
        country: "IN",
        ...
        gstHandling: gh,
        defaultCgstPercent:
          typeof o.defaultCgstPercent === "number" && Number.isFinite(o.defaultCgstPercent)
            ? o.defaultCgstPercent
            : b.defaultCgstPercent,
        defaultSgstPercent:
          typeof o.defaultSgstPercent === "number" && Number.isFinite(o.defaultSgstPercent)
            ? o.defaultSgstPercent
            : b.defaultSgstPercent,
        defaultIgstPercent:
          typeof o.defaultIgstPercent === "number" && Number.isFinite(o.defaultIgstPercent)
            ? o.defaultIgstPercent
            : b.defaultIgstPercent,
      };
    }
    case "NZ": {
...
function coerceInvoice(raw: unknown, fallback: FiTaxLocalisationDocument["invoiceSettings"]) {
  ...
  const startNum =
    typeof n === "number" && Number.isFinite(n) ? Math.floor(n) : fallback.startingInvoiceNumber;
  return {
    ...
    showTaxBreakdown:
      typeof o.showTaxBreakdown === "boolean" ? o.showTaxBreakdown : fallback.showTaxBreakdown,
...
function coerceReceipt(raw: unknown, fallback: FiTaxLocalisationDocument["receiptSettings"]) {
  ...
  return {
    legalEntityName:
      typeof o.legalEntityName === "string" ? o.legalEntityName : fallback.legalEntityName,
    tradingName: typeof o.tradingName === "string" ? o.tradingName : fallback.tradingName,
    registeredAddress:
      typeof o.registeredAddress === "string" ? o.registeredAddress : fallback.registeredAddress,
    taxFooterText: typeof o.taxFooterText === "string" ? o.taxFooterText : fallback.taxFooterText,
    refundPolicyUrl:
      typeof o.refundPolicyUrl === "string" ? o.refundPolicyUrl : fallback.refundPolicyUrl,
  };
}
...
export function mergeTaxLocalisationFromStorage(opts: {
  ...
  const country: FiTaxCountryRegion = isCountry(String(opts.countryRegion ?? ""))
    ? (opts.countryRegion as FiTaxCountryRegion)
    : "AU";
  const defaults = buildDefaultTaxLocalisationDocument(country);
  const currency = isCurrency(String(opts.currency ?? ""))
    ? (opts.currency as FiTaxLocalisationDocument["currency"])
    : defaults.currency;
  const effectiveFrom =
    typeof opts.effectiveFrom === "string" && opts.effectiveFrom.trim()
      ? opts.effectiveFrom
...
export function taxLocalisationDocumentToRowPayload(doc: FiTaxLocalisationDocument): {
  ...
  const { taxProfile, invoiceSettings, receiptSettings, countryRegion, currency, effectiveFrom } =
    doc;
  return {
