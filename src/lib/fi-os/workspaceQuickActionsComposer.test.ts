...
test("quick action composer: default preserves definition order", () => {
  const resolved = resolveDashboardQuickActions(base, {
    showCrmNav: true,
    showBookingsBoard: true,
  });
  const ordered = composeWorkspaceQuickActionsOrder({
...
test("quick action composer: director prioritises lead before booking", () => {
  ...
  assert.deepEqual(
    ordered.slice(0, 4).map((i) => i.key),
    ["lead", "booking", "case", "upload_images"]
  );
});
...
test("quick action composer: reception order", () => {
  ...
  assert.deepEqual(
    ordered.slice(0, 3).map((i) => i.key),
    ["booking", "patient", "lead"]
  );
});
