 ...
 */
export async function upsertProcedureDayForCase(
  params: UpsertProcedureDayParams,
  ...
  if (p.procedure_date !== undefined) {
    updatePayload.procedure_date = p.procedure_date?.trim()
      ? p.procedure_date.trim().slice(0, 10)
      : null;
  }
  ...
  if (p.technician_user_ids !== undefined)
    updatePayload.technician_user_ids = p.technician_user_ids;
  if (p.team_member_user_ids !== undefined)
    updatePayload.team_member_user_ids = p.team_member_user_ids;
  if (p.procedure_milestones !== undefined)
    updatePayload.procedure_milestones = p.procedure_milestones;
  if (p.procedure_location !== undefined) {
    updatePayload.procedure_location = p.procedure_location?.trim()
      ? p.procedure_location.trim()
      : null;
  }
  ...
  if (p.punch_size !== undefined)
    updatePayload.punch_size = p.punch_size?.trim() ? p.punch_size.trim() : null;
  if (p.extraction_method !== undefined) {
    updatePayload.extraction_method = p.extraction_method?.trim()
      ? p.extraction_method.trim()
      : null;
  }
  if (p.implantation_method !== undefined) {
    updatePayload.implantation_method = p.implantation_method?.trim()
      ? p.implantation_method.trim()
      : null;
  }
  ...
  if (p.intraoperative_notes !== undefined) {
    updatePayload.intraoperative_notes = p.intraoperative_notes?.trim()
      ? p.intraoperative_notes.trim()
      : null;
  }
  ...
  if (p.graft_handling_notes !== undefined) {
    updatePayload.graft_handling_notes = p.graft_handling_notes?.trim()
      ? p.graft_handling_notes.trim()
      : null;
  }
  if (p.complications_notes !== undefined) {
    updatePayload.complications_notes = p.complications_notes?.trim()
      ? p.complications_notes.trim()
      : null;
  }
  if (p.completion_summary !== undefined) {
    updatePayload.completion_summary = p.completion_summary?.trim()
      ? p.completion_summary.trim()
      : null;
  }
  ...
  const insertPayload = {
    ...
    case_id: cid,
    procedure_date:
      p.procedure_date !== undefined
        ? p.procedure_date?.trim()
          ? p.procedure_date.trim().slice(0, 10)
          : null
        : null,
    ...
    procedure_location:
      p.procedure_location !== undefined ? trimOrNull(p.procedure_location) : null,
    procedure_room: p.procedure_room !== undefined ? trimOrNull(p.procedure_room) : null,
    start_time:
      p.start_time !== undefined ? (p.start_time?.trim() ? p.start_time.trim() : null) : null,
    finish_time:
      p.finish_time !== undefined ? (p.finish_time?.trim() ? p.finish_time.trim() : null) : null,
    ...
    implantation_method:
      p.implantation_method !== undefined ? trimOrNull(p.implantation_method) : null,
    medication_notes: p.medication_notes !== undefined ? trimOrNull(p.medication_notes) : null,
    intraoperative_notes:
      p.intraoperative_notes !== undefined ? trimOrNull(p.intraoperative_notes) : null,
    grafts_extracted: p.grafts_extracted ?? null,
    grafts_implanted: p.grafts_implanted ?? null,
    hairs_implanted: p.hairs_implanted ?? null,
    graft_handling_notes:
      p.graft_handling_notes !== undefined ? trimOrNull(p.graft_handling_notes) : null,
    complications_notes:
      p.complications_notes !== undefined ? trimOrNull(p.complications_notes) : null,
    completion_summary:
      p.completion_summary !== undefined ? trimOrNull(p.completion_summary) : null,
    created_at: now,
    updated_at: now,
  };
...
async function maybeTriggerProcedureDaySnapshot(
  tenantId: string,
  caseId: string,
  patch: ProcedureDayUpsertPatch
): Promise<void> {
  ...
  try {
    await maybeTriggerSurgeryProfitabilitySnapshot({
      ...
      trigger: {
        source:
          patch.procedure_status === "completed" ? "procedure_completed" : "final_graft_count",
        metadata: {
