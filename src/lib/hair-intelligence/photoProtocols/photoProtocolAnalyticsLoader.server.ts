...
async function loadFilteredSessions(
  ...
  if (filters.source_system?.trim()) {
    q = q.eq("source_system", filters.source_system.trim() as HliPhotoProtocolSession["source_system"]);
  }
...
async function loadSessionSlotsForSessions(sessionIds: string[], client: SupabaseClient): Promise<HliPhotoProtocolSessionSlot[]> {
  if (sessionIds.length === 0) return [];
  const out: HliPhotoProtocolSessionSlot[] = [];
  const chunk = 200;
  for (let i = 0; i < sessionIds.length; i += chunk) {
    const slice = sessionIds.slice(i, i + chunk);
    const { data, error } = await client.from("hli_photo_protocol_session_slots").select("*").in("session_id", slice);
    if (error) throw new Error(error.message);
...
async function loadTemplatesAndSlots(
  templateIds: string[],
  client: SupabaseClient
): Promise<{ templatesById: Map<string, HliPhotoProtocolTemplate>; slotsByTemplateId: Map<string, HliPhotoProtocolSlot[]> }> {
  ...
  const { data: tRows, error: tErr } = await client.from("hli_photo_protocol_templates").select("*").in("id", templateIds);
  ...
  const { data: sRows, error: sErr } = await client.from("hli_photo_protocol_slots").select("*").in("protocol_template_id", templateIds);
...
async function loadPatientClinicAndNames(
  ...
  for (let i = 0; i < patientIds.length; i += chunk) {
    ...
    for (const row of data ?? []) {
      const r = row as { id: string; primary_clinic_id: string | null; person_id: string; metadata: unknown };
      clinicByPatient.set(String(r.id), r.primary_clinic_id != null ? String(r.primary_clinic_id) : null);
      personIds.add(String(r.person_id));
      const pm = r.metadata && typeof r.metadata === "object" && !Array.isArray(r.metadata) ? (r.metadata as Record<string, unknown>) : {};
    ...
    if (pids.length > 0) {
      const { data: persons, error: pe } = await client.from("fi_persons").select("id, metadata").in("id", pids);
...
export async function loadPhotoProtocolDatasetForTenant(
  ...
  const patientIds = [...new Set(sessions.map((s) => s.patient_id).filter((x): x is string => Boolean(x)))];
  const { clinicByPatient, displayByPatient } = await loadPatientClinicAndNames(tenantId, patientIds, supabase);
  return {
    sessions,
    sessionSlots,
    templatesById,
...
export async function loadIncompletePhotoProtocolSessionsForTenant(
  ...
  const ds = await loadPhotoProtocolDatasetForTenant(tenantId, mergedFilters, openStatuses, supabase);
  ...
  for (const session of ds.sessions) {
    ...
    const display = pid ? ds.patientDisplayByPatientId.get(pid) ?? `Patient ${pid.slice(0, 8)}` : "Unknown patient";
    rows.push({
      ...
      patient_twin_href: pid ? `/fi-admin/${encodeURIComponent(tid)}/patients/${encodeURIComponent(pid)}/twin` : `/fi-admin/${encodeURIComponent(tid)}/patients`,
    });
  }
