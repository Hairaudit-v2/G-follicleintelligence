...
export async function createFiOsPhotoProtocolSession(params: {
  ...
  const { data: ssIns, error: ssErr } = await supabase
    .from("hli_photo_protocol_session_slots")
    .insert(slotRows)
    .select("*");
  if (ssErr) throw new Error(ssErr.message);
  return {
    session,
    sessionSlots: (ssIns ?? []).map((x) => mapSessionSlot(x as Record<string, unknown>)),
  };
}
...
export async function attachPatientImageToSessionSlot(params: {
  ...
  if (String((sess as { tenant_id: string }).tenant_id) !== params.tenantId.trim())
  ...
  if (
    String((img as { patient_id: string }).patient_id) !==
    String((sess as { patient_id: string }).patient_id)
  ) {
  ...
  const { data: slotDef } = await supabase
    .from("hli_photo_protocol_slots")
    .select("*")
    .eq("id", String((row as { slot_id: string }).slot_id))
    .maybeSingle();
  ...
  const complianceImg = {
    ...
    ai_image_category_confidence:
      ir?.ai_image_category_confidence != null ? Number(ir.ai_image_category_confidence) : null,
    ...
    ai_image_review_status:
      ir?.ai_image_review_status != null ? String(ir.ai_image_review_status) : null,
  };
...
export async function completePhotoProtocolSessionIfEligible(params: {
  ...
  const slotIds = [
    ...new Set((ssRows ?? []).map((r) => String((r as { slot_id: string }).slot_id))),
  ];
  ...
  for (const r of ssRows ?? []) {
    ...
    if (st === "missing" || st === "needs_retake" || st === "optional_skipped")
    ...
    if (st === "captured") {
      ...
      if (conf < PROTOCOL_STRONG_CAPTURE_MIN_CONFIDENCE)
        return { ok: false, reason: "Captured slot confidence too low — accept or retake." };
    } else {
      return { ok: false, reason: "Required slots are not satisfied." };
    }
  }
