...
import { HIE_CONSULTATION_CONSENT_COMPLEXITY_LEVELS, HIE_CONSULTATION_PRIORITY_LEVELS } from "./types";
...
function normPriority(p: string): HieConsultationPriorityLevel {
  return (HIE_CONSULTATION_PRIORITY_LEVELS as readonly string[]).includes(p) ? (p as HieConsultationPriorityLevel) : "low";
}

function normConsent(c: string | null | undefined): HieConsultationConsentComplexityLevel | "unset" {
...
export function consultationChecklistPriorityDistribution(rows: ConsultationChecklistAnalyticsRow[]): Record<HieConsultationPriorityLevel, number> {
...
export function consultationChecklistDelayRecommendationRate(rows: ConsultationChecklistAnalyticsRow[]): number {
  if (rows.length === 0) return 0;
  const n = rows.filter((r) => r.delay_recommended).length;
  return n / rows.length;
}

export function consultationChecklistExpectationManagementRate(rows: ConsultationChecklistAnalyticsRow[]): number {
  if (rows.length === 0) return 0;
  const n = rows.filter((r) => r.expectation_management_required).length;
  return n / rows.length;
}

export function consultationChecklistMedicationDiscussionRate(rows: ConsultationChecklistAnalyticsRow[]): number {
  if (rows.length === 0) return 0;
...
export function consultationChecklistConsentComplexityDistribution(
  ...
  const out = Object.fromEntries(keys.map((k) => [k, 0])) as Record<HieConsultationConsentComplexityLevel | "unset", number>;
  for (const r of rows) {
...
export function consultationChecklistRiskFlagFrequency(rows: ConsultationChecklistAnalyticsRow[]): Record<string, number> {
