...
test("computeEffectivePaymentStatus: paid unchanged", () => {
  assert.equal(
    computeEffectivePaymentStatus({ status: "paid", due_date: "2020-01-01" }, "2026-06-10"),
    "paid"
  );
});
...
test("computeEffectivePaymentStatus: overdue derived from due_date", () => {
  assert.equal(
    computeEffectivePaymentStatus({ status: "pending", due_date: "2026-06-01" }, "2026-06-10"),
    "overdue_derived"
  );
  assert.equal(
    computeEffectivePaymentStatus(
      { status: "partially_paid", due_date: "2026-06-09" },
      "2026-06-10"
    ),
    "overdue_derived"
  );
...
test("computeEffectivePaymentStatus: pending future due is not overdue", () => {
  assert.equal(
    computeEffectivePaymentStatus({ status: "pending", due_date: "2026-06-20" }, "2026-06-10"),
    "pending"
  );
});
...
test("paymentRecordNeedsCollection: waived is false", () => {
  assert.equal(
    paymentRecordNeedsCollection(
      { status: "waived", due_date: null, amount_expected: 100, amount_paid: 0 },
      "2026-06-10"
    ),
    false
  );
