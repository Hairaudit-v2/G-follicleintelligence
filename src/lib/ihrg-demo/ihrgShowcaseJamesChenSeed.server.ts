...
import {
  ...
  ENTERPRISE_DEMO_PATIENT_KEY_METADATA,
...
import { SHOWCASE_JAMES_CHEN_PATIENT_KEY, SHOWCASE_PACKAGE_A } from "@/src/lib/demo-day";
import {
...
const SOURCE_SYSTEM = "titan_demo";
const PROTOCOL_TEMPLATE = "titan_surgery_outcome";
const SHOWCASE_FLAG = SHOWCASE_PACKAGE_A.showcaseFlag;

export type IhrgShowcaseJamesChenSeedResult = {
  ok: boolean;
...
function makeResult(): IhrgShowcaseJamesChenSeedResult {
  return {
    ok: true,
    patientId: null,
    personId: null,
...
function finish(result: IhrgShowcaseJamesChenSeedResult): IhrgShowcaseJamesChenSeedResult {
...
async function ensurePersonSource(
  ...
  tenantId: string,
  personId: string,
  result: IhrgShowcaseJamesChenSeedResult
): Promise<void> {
  const { data, error } = await supabase
...
async function seedFinancials(opts: {
  ...
  caseId: string;
  nowIso: string;
  spec: ReturnType<typeof buildIhrgShowcaseJamesChenSpec>;
  result: IhrgShowcaseJamesChenSeedResult;
}): Promise<boolean> {
  const invoices = [
    {
  ...
  for (const invoice of invoices) {
    ...
    if (existing) {
      ...
      const number = `TITAN-${invoice.key.replace(/[^a-z0-9]+/gi, "-").slice(0, 42)}`.toUpperCase();
...
export async function seedIhrgShowcaseJamesChen(
  supabase: SupabaseClient,
  tenantId: string,
  now: Date = new Date()
): Promise<IhrgShowcaseJamesChenSeedResult> {
  const result = makeResult();
  const spec = buildIhrgShowcaseJamesChenSpec(now);
  const nowIso = now.toISOString();
  try {
    assertIhrgShowcaseJamesSpecConsistent(spec);
    ...
    const clinic = (clinics ?? []).find(
      (row) => asRecord((row as { metadata?: unknown }).metadata).slug === spec.clinicSlug
    ) as { id: string } | undefined;
    if (!clinic) throw new Error(`Clinic slug "${spec.clinicSlug}" not found.`);
    ...
    if (person && !isDemo(person.metadata, "enterprise_demo_person")) {
      result.warnings.push(
    ...
    if (patient && !isDemo(patient.metadata, ENTERPRISE_DEMO_PATIENT_METADATA_FLAG)) {
      result.warnings.push(
    ...
    if (!consultation) {
      const consultant = await findByJsonKey(
        supabase,
        "fi_staff",
        tenantId,
        "staff_metadata",
        "demo_staff_key",
        spec.staffKeys.consultant,
        "id"
      );
      const { data, error } = await supabase
        ...
        .insert({
          ...
          consultant_staff_id: consultant?.id ?? null,
    ...
    for (const raw of staffData ?? []) {
      const staff = raw as StaffRow;
      const key = asRecord(staff.staff_metadata).demo_staff_key;
      if (typeof key === "string") staffByKey.set(key, staff);
    }
    const userByKey = new Map<string, string>();
    for (const key of [spec.staffKeys.surgeon, spec.staffKeys.nurse, spec.staffKeys.technician]) {
      const staff = staffByKey.get(key);
      if (!staff) {
        result.warnings.push(`Staff "${key}" not found.`);
        continue;
      }
    ...
    for (const slot of IHRG_SHOWCASE_JAMES_IMAGE_SLOTS) {
      const imageKey = demoImageKeyForSlot(slot);
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          storage_path: `titan-demo/synthetic/${imageKey}.jpg`,
    ...
    if (protocol) {
      ...
      const progress: JsonRecord = {
        _demo: {
          ...
          slots_expected: IHRG_SHOWCASE_JAMES_IMAGE_SLOTS.length,
    ...
    result.fixturePresence.baseline =
      ...
      imageIds.size === IHRG_SHOWCASE_JAMES_IMAGE_SLOTS.length;
