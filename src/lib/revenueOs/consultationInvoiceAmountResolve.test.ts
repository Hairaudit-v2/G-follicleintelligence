...
import {
  resolveConsultationQuoteInvoiceAmountCents,
  resolveConsultationQuoteInvoiceSource,
} from "./consultationInvoiceAmountResolve";

test("prefers consultation quote_data.price_quoted", () => {
  const cents = resolveConsultationQuoteInvoiceAmountCents(
    { quote_data: { price_quoted: "$1,200 AUD" } },
    []
  );
...
test("uses CRM quote subtotal_amount (dollars) when panel empty", () => {
  const cents = resolveConsultationQuoteInvoiceAmountCents({ quote_data: {} }, [
...
test("resolveConsultationQuoteInvoiceSource: panel amount links primary (first) CRM quote id", () => {
  ...
  const src = resolveConsultationQuoteInvoiceSource({ quote_data: { price_quoted: "$500 AUD" } }, [
...
test("resolveConsultationQuoteInvoiceSource: CRM subtotal carries that quote id for invoice metadata", () => {
  ...
  const src = resolveConsultationQuoteInvoiceSource({ quote_data: {} }, [
