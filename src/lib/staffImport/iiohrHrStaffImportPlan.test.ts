...
test("terminated status deactivates staff", () => {
  ...
  assert.ok(de);
  assert.equal(de.type === "align_iiohr_departure" && de.payload.staffId, "staff-t");
  assert.equal(de.type === "align_iiohr_departure" && de.payload.hrEmploymentStatus, "terminated");
