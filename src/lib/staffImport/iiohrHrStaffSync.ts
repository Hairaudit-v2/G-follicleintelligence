...
export function mapIiohrHrStaffSyncRowToImportRow(row: IiohrHrStaffSyncRow): IiohrHrStaffImportRow {
  return {
    ...
    email:
      row.email != null && String(row.email).trim() ? String(row.email).trim().toLowerCase() : null,
    ...
    default_timezone:
      row.default_timezone != null ? String(row.default_timezone).trim() : undefined,
    working_hours: row.working_hours,
    iiohr_user_id:
      row.iiohr_user_id != null && String(row.iiohr_user_id).trim()
        ? String(row.iiohr_user_id).trim()
        : null,
  };
}

function combinedSnapshotFromSyncRow(
  row: IiohrHrStaffSyncRow | undefined
): Record<string, unknown> {
  if (!row) return {};
  const combined: Record<string, unknown> = {};
  if (
    row.metadata_snapshot &&
    typeof row.metadata_snapshot === "object" &&
    !Array.isArray(row.metadata_snapshot)
  ) {
  ...
  if (row.onboarding_completed_at !== undefined)
    combined.onboarding_completed_at = row.onboarding_completed_at;
  ...
  if (row.training_required_count !== undefined)
    combined.training_required_count = row.training_required_count;
  ...
  for (const pr of plan.perRow) {
    for (const a of pr.actions) {
      if (a.type === "create_staff_source_id") {
        ...
        a.payload.metadata = mergeHrStaffSourceMetadataOnSync(
          base,
          snap,
          lastSyncedAt,
          syncRow?.source_url
