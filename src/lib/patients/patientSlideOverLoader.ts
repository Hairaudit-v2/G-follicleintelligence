...
function mapBookingRow(raw: Record<string, unknown>): FiBookingRow {
  ...
  return {
    ...
    room_id: raw.room_id != null ? String(raw.room_id) : null,
    room_required: raw.room_required == null ? true : Boolean(raw.room_required),
    ...
    cancelled_by_user_id:
      raw.cancelled_by_user_id != null ? String(raw.cancelled_by_user_id) : null,
...
export async function loadPatientSlideOverPayload(
  ...
  const [
    ...
    supabase
      ...
      .select(
        "norwood_scale, ludwig_scale, hairline_pattern, primary_concern, primary_hair_concern"
      )
      .eq("tenant_id", tid)
      .eq("patient_id", pid)
      .maybeSingle(),
  ...
  const meta =
    personRow?.metadata &&
    typeof personRow.metadata === "object" &&
    !Array.isArray(personRow.metadata)
  ...
  return {
    ...
    email,
    phone,
    patientStatus: normalizePatientStatus(
      (patRow as { patient_status?: string | null }).patient_status
    ),
