...
test("canonicaliseWorkforceSourceSystem maps legacy aliases", () => {
  ...
  assert.equal(canonicaliseWorkforceSourceSystem("iiohr"), WORKFORCE_IDENTITY_SOURCE_SYSTEMS.IIOHR_HR);
  assert.equal(canonicaliseWorkforceSourceSystem("academy"), WORKFORCE_IDENTITY_SOURCE_SYSTEMS.IIOHR_ACADEMY);
  assert.equal(canonicaliseWorkforceSourceSystem("nexus"), WORKFORCE_IDENTITY_SOURCE_SYSTEMS.IIOHR_NEXUS);
  assert.equal(canonicaliseWorkforceSourceSystem("IIOHR_HR"), WORKFORCE_IDENTITY_SOURCE_SYSTEMS.IIOHR_HR);
});
