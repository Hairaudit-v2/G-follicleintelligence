...
const SURGICAL_CATEGORIES = new Set(["pre_op_vs_post_op", "recipient_before_vs_after_implantation"]);
...
export function pairContributesToOutcomeEvidence(pair: VieOutcomeComparisonInput): VieOutcomePairContribution {
  ...
  if (domains.length === 0) {
    return { contributes: false, domains: [], reason: "Pair category does not map to an outcome domain" };
  }
...
function domainsForPair(pair: VieOutcomeComparisonInput): VieOutcomeDomain[] {
  ...
  if (AUDIT_CATEGORIES.has(pair.comparison_category) || pair.recommended_use.includes("audit_evidence")) {
...
function scoreFromPairs(pairs: VieOutcomeComparisonInput[]): { score: number; evidenceCount: number; hasConcern: boolean; bestPairId: string | null } {
...
function scoreDonorRecovery(
  ...
  const combinedScore = fromPairs.evidenceCount > 0 ? Math.round(fromPairs.score * 0.65 + docBoost * 0.35) : docBoost;
...
function scoreAuditEvidence(
  ...
  const auditPairs = pairs.filter(
    (p) =>
      AUDIT_CATEGORIES.has(p.comparison_category) ||
...
function buildDomainSummary(
  ...
  const status = deriveDomainStatus(input.score, input.evidenceCount, input.hasConcern, auditDomain);
  return {
    domain,
    score: input.score,
    status,
...
function deriveOverallConfidence(
  ...
  const highConf = eligible.filter((p) => p.confidence_band === "high" || p.review_status === "accepted").length;
...
export function buildVieOutcomeSummary(input: {
  ...
  const domainInputs: Array<{ domain: VieOutcomeDomain; data: typeof recipient; audit?: boolean; extraWarnings?: string[] }> = [
    { domain: "recipient_growth", data: recipient, extraWarnings: recipient.hasConcern ? ["Alignment or quality concern in recipient progression pairs"] : [] },
    { domain: "donor_recovery", data: donor, extraWarnings: completeness.donor_documentation_percent < 50 ? ["Donor documentation incomplete"] : [] },
    { domain: "hairline_maturation", data: hairline },
    { domain: "crown_progress", data: crown },
    { domain: "surgical_healing", data: surgical, extraWarnings: completeness.surgical_documentation_percent < 50 ? ["Surgical day documentation incomplete"] : [] },
    {
      domain: "documentation_readiness",
      data: { ...documentation, bestPairId: null },
      extraWarnings: documentation.score < 50 ? ["Protocol completeness below monitoring threshold"] : [],
    },
  ...
  const overall_outcome_readiness_score = weightTotal > 0 ? Math.round(weightedSum / weightTotal) : 0;
  ...
  if (pairs.filter(isOutcomeEligiblePair).length === 0) {
    globalWarnings.push("No accepted or high-confidence comparison pairs available for outcome evidence");
  }
