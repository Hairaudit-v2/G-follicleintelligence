...
import {
  ...
  serviceForCatalogName,
} from "./servicesCatalog";

function row(partial: Partial<FiServiceRow> & Pick<FiServiceRow, "name" | "booking_type">): FiServiceRow {
  return {
    id: "id",
    tenant_id: "t1",
...
describe("quickTemplateDurationMinutes", () => {
  it("uses catalog name match before booking_type", () => {
    const services = [
      row({ name: "In-Clinic Consultation", booking_type: "consultation", category: "Consultation", duration_minutes: 45 }),
      row({ name: "Phone Consultation", booking_type: null, category: "Consultation", duration_minutes: 25 }),
    ];
  ...
  it("resolves PRP duration from fi_services", () => {
    const services = [row({ name: "PRP Treatment", booking_type: "prp", category: "Treatment", duration_minutes: 75 })];
