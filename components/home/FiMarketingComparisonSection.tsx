...
const tdBase = "border-b border-white/[0.06] px-3 py-3 align-top text-[13px] leading-snug sm:px-4 sm:py-3.5 sm:text-sm sm:leading-relaxed";
const rowLabel =
    ...
    >
      <FadeIn>
        ...
        <div className="mt-12 sm:mt-14">
            ...
            <div className="relative overflow-x-auto [-webkit-overflow-scrolling:touch]">
              <table className="relative w-full min-w-[42rem] border-separate border-spacing-0 sm:min-w-[52rem]">
                <caption className="sr-only">
                  Comparison of Traditional CRM, Generic Clinic Software, and Follicle Intelligence across clinical and
                  intelligence capabilities.
                </caption>
                <thead>
                  <tr>
                    <th scope="col" className={cn(thBase, "w-[min(22%,14rem)] rounded-tl-[1.2rem] bg-white/[0.03] text-muted-foreground")}>
                      Capability
                    </th>
                    ...
                    >
                      <span className="inline-flex items-center gap-2">
                        <CheckCircle2 className="h-3.5 w-3.5 shrink-0 text-amber-300/90 sm:h-4 sm:w-4" aria-hidden />
                        {colFi}
                      </span>
                    </th>
                  </tr>
                ...
                <tbody>
                  {section.rows.map((row, idx) => {
                    ...
                    return (
                      <tr key={row.capability} className="transition-colors duration-200 hover:bg-white/[0.02]">
                        <th scope="row" className={cn(rowLabel, isLast && "rounded-bl-[1.2rem] border-b-0")}>
                          {row.capability}
                        </th>
                        <td className={cn(tdBase, "text-muted-foreground", isLast && "border-b-0")}>{row.traditionalCrm}</td>
                        <td className={cn(tdBase, "text-muted-foreground", isLast && "border-b-0")}>{row.genericClinic}</td>
                        <td
                          className={cn(
                            tdBase,
                            ...
                            isLast && "rounded-br-[1.2rem] border-b-0 shadow-[inset_0_-1px_0_rgb(212_175_55_/0.12)]"
                          )}
                        >
          ...
          <p className="mt-4 text-center text-xs leading-relaxed text-muted-foreground/90 sm:text-sm">{section.footnote}</p>
        </div>
      </FadeIn>
    </Section>
  );
