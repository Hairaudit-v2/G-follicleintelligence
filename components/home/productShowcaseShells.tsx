...
export function ProductShowcaseShell({ variant }: { variant: HomeProductShowcaseShell }) {
  switch (variant) {
    case "clinic-calendar":
      return (
        <ShellChrome>
          <TopBar title="ClinicOS · Schedule" />
    ...
    case "crm-pipeline":
      return (
        <ShellChrome>
          <TopBar title="LeadFlowOS · Pipeline" />
          <div className="flex h-[calc(100%-1.75rem)] gap-1.5 p-2 sm:gap-2 sm:p-2.5">
            {[
              { label: "New", n: 4, hot: 1 },
    ...
    case "consultation-workflow":
      return (
        <ShellChrome>
          <TopBar title="ConsultationOS · Workflow" />
          <div className="flex h-[calc(100%-1.75rem)] gap-2 p-2 sm:p-2.5">
            <div className="flex w-[38%] max-w-[7.5rem] flex-col gap-1 rounded-lg border border-white/[0.06] bg-white/[0.02] p-1.5">
              {["Intake", "Exam", "Plan", "Consent"].map((step, i) => (
                <div
                  key={step}
                  className={cn(
            ...
            <div className="flex min-w-0 flex-1 flex-col gap-1.5 rounded-lg border border-white/[0.06] bg-[rgb(7_10_16_/0.65)] p-2">
              ...
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="h-0.5 w-full rounded bg-white/[0.05]" style={{ width: `${68 + (i % 3) * 8}%` }} />
              ))}
    ...
    case "surgical-planning":
      return (
        <ShellChrome>
          <TopBar title="SurgeryOS · Planning" />
    ...
    case "metrics-dashboard":
      return (
        <ShellChrome>
          <TopBar title="AnalyticsOS · Command" />
          <div className="flex h-[calc(100%-1.75rem)] flex-col gap-2 p-2 sm:p-2.5">
            <div className="grid grid-cols-3 gap-1.5">
              {["Rev", "Conv", "Prod"].map((k) => (
                <div key={k} className="rounded-lg border border-white/[0.06] bg-white/[0.03] p-1.5">
