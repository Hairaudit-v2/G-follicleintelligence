    ...
    <AnimatePresence>
      {open ? (
        <motion.aside
          ...
          className={cn(
            "fixed bottom-4 left-4 z-50 w-[min(100vw-2rem,18rem)] rounded-2xl border border-white/[0.08] bg-white/95 p-4 shadow-xl backdrop-blur-md",
            className
          )}
        ...
        >
          <div className="mb-3 flex items-center justify-between gap-2">
            ...
            <button
              type="button"
              onClick={onClose}
              className="rounded-lg p-1 text-slate-500 transition hover:bg-white/[0.06] hover:text-slate-200"
              ...
              <li key={row.label} className="flex items-center justify-between gap-3 text-xs">
                ...
                <span className="flex shrink-0 gap-1">
                  {row.keys.map((k) => (
                    <kbd
                      key={k}
                      className="rounded-md border border-white/[0.08] bg-white/[0.03] px-1.5 py-0.5 font-mono text-[10px] font-semibold text-slate-300"
                    >
                      {k}
                    </kbd>
                  ))}
