    ...
    <>
      ...
      >
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-20 md:py-24">
          <FadeIn>
            <p className="text-[10px] font-semibold uppercase tracking-[0.34em] text-amber-200/85 sm:text-[11px]">{c.hero.eyebrow}</p>
            <div className="mt-3 h-px w-14 bg-gradient-to-r from-amber-300/65 via-amber-400/22 to-transparent" aria-hidden />
            <h1
            ...
            {c.problem.cards.map((card, i) => (
              <li key={card.title}>
                <FadeIn delay={0.04 * (i % 4)}>
                  <GlassCard variant="problem" className="flex h-full flex-col border-amber-400/[0.07]">
                    <h3 className="font-display text-base font-semibold leading-snug tracking-tight text-foreground md:text-lg">{card.title}</h3>
      ...
      >
        <FadeIn>
          <SectionHeading id={`${c.iihr.id}-heading`} eyebrow={c.iihr.eyebrow} title={c.iihr.headline} />
      ...
      >
        <FadeIn>
          <SectionHeading id={`${c.trainingTracks.id}-heading`} eyebrow={c.trainingTracks.eyebrow} title={c.trainingTracks.headline} />
                ...
                <FadeIn delay={0.04 * (i % 4)}>
                  ...
                  >
                    <div className="flex items-center gap-2 border-b border-white/[0.06] pb-2.5">
                      ...
                      <span className="h-px flex-1 bg-gradient-to-r from-indigo-400/20 via-amber-400/15 to-transparent" aria-hidden />
                    </div>
                    <h3 className="mt-3 font-display text-base font-semibold tracking-tight text-foreground md:text-lg">{card.title}</h3>
                    <p className="mt-3 flex-1 text-sm leading-relaxed text-muted-foreground md:text-[0.9375rem]">{card.body}</p>
                  </GlassCard>
                </FadeIn>
              </li>
            ))}
      ...
      >
        <FadeIn>
          <SectionHeading id={`${c.competency.id}-heading`} eyebrow={c.competency.eyebrow} title={c.competency.headline} />
            ...
            {c.competency.cards.map((card, i) => (
              <li key={card.title}>
                <FadeIn delay={0.03 * (i % 5)}>
                  <GlassCard variant="os" className="flex h-full flex-col !rounded-[1.25rem] border-white/[0.06] !p-5 sm:!p-5">
                    <h3 className="font-display text-sm font-semibold leading-snug tracking-tight text-foreground sm:text-base">{card.title}</h3>
                    <p className="mt-2.5 text-xs leading-relaxed text-muted-foreground sm:text-sm">{card.body}</p>
                  </GlassCard>
                </FadeIn>
              </li>
            ))}
      ...
      >
        <FadeIn>
          <SectionHeading id={`${c.connected.id}-heading`} eyebrow={c.connected.eyebrow} title={c.connected.headline} />
            ...
            <ul className="mt-8 grid list-none gap-3 border-t border-white/[0.06] p-0 pt-8 sm:grid-cols-1 md:grid-cols-2">
              {c.connected.signals.map((signal) => (
                <li key={signal} className="flex gap-3 text-sm leading-snug text-foreground/90">
                  <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-indigo-400/45" aria-hidden />
                  <span>{signal}</span>
                </li>
              ))}
            </ul>
          ...
          <FadeIn>
            <div className="rounded-[1.75rem] border border-white/[0.1] bg-gradient-to-br from-white/[0.05] via-indigo-950/[0.04] to-transparent p-7 shadow-[0_28px_90px_rgb(0_0_0_/0.45),inset_0_1px_0_rgb(255_255_255_/0.05)] backdrop-blur-md sm:rounded-[2rem] sm:p-10 md:p-12">
              <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-amber-200/80">{c.finalCta.eyebrow}</p>
                  <div className="mt-3 h-px w-14 bg-gradient-to-r from-amber-300/65 via-amber-400/22 to-transparent" aria-hidden />
                  <h2
