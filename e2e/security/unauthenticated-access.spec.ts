...
test.describe("unauthenticated access — fails closed @security", () => {
  ...
  test("protected tenant patients API fails closed without a session", async ({ request }) => {
    const tenantId = e2eTenantId();
    // Collection path has no Route Handler (only /patients/[patientId]/…);
    // Next returns 404 — intentional fail-closed (PUB-SEC-STATUS / Bucket C).
    ...
    expect(
      [401, 403, 404, 302, 303, 307].includes(status),
      `expected 401/403/404 (or redirect) for unauthenticated patients API, got ${status}`
    ).toBe(true);
  });
  ...
  test("cron reminder route rejects unauthenticated calls", async ({ request }) => {
    ...
    // 503 = assertCronAuthorized when CRON / reminder secrets unset (CI placeholder);
    // 401/403 = secret configured but caller unauthorized; 500 = processor fault.
    expect(
      [401, 403, 500, 503].includes(status),
      `expected 401/403/500/503 for unauthenticated cron, got ${status}`
    ).toBe(true);
  });
