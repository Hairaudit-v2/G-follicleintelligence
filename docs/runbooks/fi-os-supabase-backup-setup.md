...
- [ ] **Before restore:** register/verify a **recovery marker** on production (non-PHI / `SMOKETEST-` synthetic row). There is no separate `fi_restore_drill_markers` insert table — seed a dated Evolved lead if the previous marker is outside the **7-day PITR** window, then run the read-only verify script:
  ...
  Record marker id / row id / `created_at` in the [backup audit](../production/evidence/backup-disaster-recovery-audit.md). After staging restore, re-run the same check (or staging SQL) against the **staging** DB. Choose a PITR timestamp **after** the marker and **within 7-day retention**. **Never** restore onto production. **Do not** extend retention for the drill unless product policy changes.
