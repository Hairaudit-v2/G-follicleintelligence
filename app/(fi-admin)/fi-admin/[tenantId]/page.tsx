...
import { fiDashboardWidgetVisibleByFeatureAccess, filterResolvedQuickActionsByFeatureAccess } from "@/src/lib/fi-os/stage2FeatureVisibility";
...
export default async function FiAdminTenantHomePage({ params }: { params: Promise<{ tenantId: string }> }) {
  ...
  const [showCrmNav, showBookingsBoard, featureAccess, workspaceProfile, showSystemDiagnostics] = await Promise.all([
  ...
  const shouldLoadClinical =
    showSystemDiagnostics && fiDashboardWidgetVisibleByFeatureAccess("clinical_intelligence_summary", featureAccess);
  const shouldLoadOutcome =
    showSystemDiagnostics && fiDashboardWidgetVisibleByFeatureAccess("outcome_intelligence_summary", featureAccess);
