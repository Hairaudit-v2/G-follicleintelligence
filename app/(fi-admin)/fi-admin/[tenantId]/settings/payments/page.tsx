...
export default async function FiOsPaymentsSettingsPage({
  params,
}: {
    ...
    <div className="mx-auto max-w-3xl space-y-6 py-8 text-slate-100">
        ...
        <dl className="mt-3 space-y-2 text-sm text-slate-300">
          ...
          <div className="flex justify-between gap-4">
            ...
            <dd className="font-mono text-cyan-200">
              {stripeMode ? "Stripe checkout" : "Manual / link-only"}
            </dd>
          </div>
          <div className="flex justify-between gap-4">
      ...
      <section className="rounded-xl border border-white/[0.08] bg-[#0b1220]/80 p-5">
        ...
        <p className="mt-2 text-sm text-slate-400">
          Public patient pages use opaque tokens at{" "}
          <code className="rounded bg-black/40 px-1">/pay/[token]</code> — no tenant dashboard
          sign-in required.
        </p>
        <dl className="mt-3 space-y-2 text-sm text-slate-300">
          <div className="flex justify-between gap-4">
            ...
            <dd className="font-mono text-cyan-200">
              {defaultExpiryDays}{" "}
              <span className="text-xs text-slate-500">(FI_PAYMENT_REQUEST_EXPIRY_DAYS)</span>
            </dd>
          </div>
        </dl>
      </section>
      ...
      <section className="rounded-xl border border-white/[0.08] bg-[#0b1220]/80 p-5">
        ...
        <dl className="mt-3 space-y-2 text-sm text-slate-300">
          <div className="flex justify-between gap-4">
            ...
            <dd className="max-w-[60%] truncate font-mono text-xs text-slate-400">
              {successUrl ?? "—"}
            </dd>
          </div>
          <div className="flex justify-between gap-4">
