...
export default async function PlatformDeploymentsPage() {
  ...
  if (!loaded.ok) {
    return <p className="text-sm text-red-400">Could not load deployment dashboard: {loaded.error}</p>;
  }
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 max-w-2xl text-sm text-slate-500">
          Active onboarding deployments scored across infrastructure, workflows, staff, operational testing, adoption
          confidence, and executive approval. Go-live approval remains a separate explicit platform action.
        </p>
      </div>
          ...
          <div className="mt-3 overflow-x-auto rounded-xl border border-white/[0.08] bg-[#060d18]/80">
            <table className="min-w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-white/[0.06] text-slate-300">
                {rows.map((row) => {
                  const provBadge = resolveProvisioningStatusBadge(row.provisioningStatus as ProvisioningSessionStatus);
                  ...
                  return (
                    <tr key={row.sessionId}>
                      ...
                      <td className="px-4 py-3">
                        ...
                        <p className="mt-1 text-xs text-slate-600">{row.provisioningProgressPercent}%</p>
                      </td>
                      <td className="px-4 py-3 font-mono font-semibold text-cyan-300">{row.deploymentScore}%</td>
                      <td className="px-4 py-3">
                        <span
                      ...
                      <td className="px-4 py-3 font-mono text-xs">{row.adoptionConfidenceScore}%</td>
                      <td className="px-4 py-3 text-xs capitalize">{GO_LIVE_STATE_LABELS[row.goLiveApprovalState] ?? row.goLiveApprovalState}</td>
                      <td className="px-4 py-3">
                        <Link
