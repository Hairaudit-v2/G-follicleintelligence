...
} from "@/src/lib/fi/events/governedReplayEnv";
    ...
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-1 text-xl font-semibold tracking-tight text-slate-50">
          Governed replay runs (Stage 15 / 17)
        </h1>
        <p className="mt-2 max-w-2xl text-sm text-slate-400">
          Durable plans in{" "}
          <code className="text-xs text-slate-300">public.fi_intelligence_replay_runs</code> with
          draft → approval → execute. Execution stays on safe Stage 14 modes only;{" "}
          <strong className="text-slate-200">dispatch_future</strong> is blocked in app code. No
          production dispatch controls are exposed here.
        </p>
        <p className="mt-2 text-xs text-slate-500">
          <Link
            href="/fi-admin/system/intelligence-event-logs"
      ...
      <aside className="rounded-lg border border-amber-500/25 bg-amber-500/[0.06] p-3 text-xs text-amber-100/90">
        <strong className="text-amber-100">Production governance required.</strong> Cross-system
        dispatch and production persistence remain off until org sign-off.{" "}
        <strong className="text-amber-100">No production activation</strong> for staging replay
        rehearsal — follow{" "}
        ...
        <ul className="mt-2 list-inside list-disc space-y-1 text-slate-400">
          <li>
            ...
            <strong className="text-slate-200">
              {governedReplay ? "on (execute allowed)" : "off (default)"}
            </strong>
          </li>
          <li>
            ...
            — <strong className="text-slate-200">{governedDispatchFlag ? "set" : "unset"}</strong>{" "}
            (reserved; no Stage 15 dispatch implementation)
          </li>
          <li>
            ...
            {isProd ? (
              <span className="text-amber-300/90">
                {" "}
                — production: shadow enqueue remains hard-off per Stage 12/14 policy.
              </span>
            ) : null}
          </li>
          <li>
            <code className="text-xs text-slate-300">
              FI_INTELLIGENCE_STAGING_ACTIVATION_ENABLED
            </code>{" "}
            + <code className="text-xs text-slate-300">FI_INTELLIGENCE_STAGING_ALLOWED_EVENT</code>{" "}
            —{" "}
            <strong className="text-slate-200">
              {stagingActivation ? "staging path eligible" : "off (default)"}
            </strong>
            {isProd ? (
              <span className="text-amber-300/90">
                {" "}
                — staging activation is always false in production.
              </span>
            ) : null}
          </li>
        </ul>
      ...
      <section className="rounded-xl border border-cyan-500/20 bg-cyan-500/[0.04] p-4 text-sm text-slate-300">
        <h2 className="text-xs font-semibold uppercase tracking-wide text-cyan-200/90">
          Stage 17 — staging activation (no UI trigger)
        </h2>
        <p className="mt-2 text-slate-400">
          One explicit staging-only path unlocks <strong className="text-slate-200">CLI</strong>{" "}
          execution of an <strong className="text-slate-200">approved</strong> governed run in{" "}
          <strong className="text-slate-200">enqueue_shadow</strong> only, for allow-listed events
          below. There is <strong className="text-slate-200">no browser activation button</strong>{" "}
          yet; use env flags and the operator script. No downstream dispatch is added; production
          stays disabled.
        </p>
        <ul className="mt-2 list-inside list-disc space-y-1 text-slate-400">
          <li>
            Required when using the staging path:{" "}
            <code className="text-xs text-slate-300">NODE_ENV</code> ≠ production,{" "}
            <code className="text-xs text-slate-300">
          ...
          <li>
            ...
            <code className="text-xs text-slate-300">docs/stage17-staging-activation-path.md</code>,
            inspect replay run + logs, restart if you need to clear the process-local queue.
          </li>
          <li>
      ...
      <section className="rounded-xl border border-white/[0.08] bg-[#060d18]/80 p-4 text-sm text-slate-300">
        ...
        <p className="mt-2 text-slate-400">
          Write actions use the same service-role script as Stage 14. Governed{" "}
          <strong className="text-slate-200">execute</strong> requires{" "}
        ...
        {runs.error ? (
          <p className="mt-2 text-red-400">
            ...
            <span className="mt-1 block text-xs text-slate-500">
              ...
              <code className="text-slate-400">20260818120001_fi_intelligence_replay_runs</code>.
            </span>
          </p>
        ) : runs.rows.length === 0 ? (
