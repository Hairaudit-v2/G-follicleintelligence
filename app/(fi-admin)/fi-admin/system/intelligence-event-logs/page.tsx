...
export default async function IntelligenceEventLogsPage() {
  const [{ rows, error, persistFlagSet, persistEffective }, replayDryRun, compare] =
    await Promise.all([
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-2xl text-sm text-slate-400">
          Read-only view of{" "}
          <code className="text-xs text-slate-300">public.fi_intelligence_event_logs</code>. Rows
          contain sanitized metadata only (no raw clinical payloads). Persistence is{" "}
          <strong className="text-slate-200">
            {persistEffective ? "effective in this environment" : "off"}
          </strong>
          {persistFlagSet && !persistEffective
            ? " (flag is set but production or policy gate forces off)"
            : null}
          .
        </p>
      </div>
      ...
      <section className="rounded-xl border border-white/[0.08] bg-[#060d18]/80 p-4 text-sm text-slate-300">
        <h2 className="text-xs font-semibold uppercase tracking-wide text-slate-500">
          Stage 14 replay filters
        </h2>
        <p className="mt-2 text-slate-400">
          Supported candidate filters (CLI and server helpers):{" "}
          <code className="text-xs text-slate-300">event_name</code>,{" "}
          <code className="text-xs text-slate-300">source</code>,{" "}
          <code className="text-xs text-slate-300">status</code>,{" "}
          <code className="text-xs text-slate-300">privacy_level</code>,{" "}
          <code className="text-xs text-slate-300">since</code> /{" "}
          <code className="text-xs text-slate-300">until</code> on{" "}
          <code className="text-xs text-slate-300">created_at</code>,{" "}
          <code className="text-xs text-slate-300">correlation_id</code>,{" "}
          <code className="text-xs text-slate-300">limit</code> (1–500),{" "}
          <code className="text-xs text-slate-300">order</code>{" "}
          <span className="text-slate-500">(newest_first | oldest_first)</span>. Default replay mode
          is dry-run (no writes).
        </p>
      ...
      <section className="rounded-xl border border-white/[0.08] bg-[#060d18]/80 p-4 text-sm text-slate-300">
        <h2 className="text-xs font-semibold uppercase tracking-wide text-slate-500">
          Replay dry-run (latest window)
        </h2>
        {replayDryRun.load_error ? (
          <p className="mt-2 text-red-400">Replay load error: {replayDryRun.load_error}</p>
        ) : (
          <p className="mt-2 text-slate-400">
            Mode <code className="text-xs text-slate-200">{replayDryRun.summary.mode}</code> —
            candidates loaded:{" "}
            <strong className="text-slate-200">{replayDryRun.summary.candidates_loaded}</strong>{" "}
            (limit 25, newest first).
          </p>
        )}
        ...
        {compare.error ? (
          ...
          <div className="mt-2 space-y-2 text-slate-400">
            <p>
              Intelligence rows:{" "}
              <strong className="text-slate-200">
                {compare.summary.intelligence_rows_sampled}
              </strong>{" "}
              — <code className="text-xs text-slate-300">fi_events</code> rows:{" "}
              <strong className="text-slate-200">{compare.summary.fi_events_rows_sampled}</strong>.
              Field names:{" "}
            ...
            {compare.summary.event_names_only_in_intelligence.length > 0 ? (
              <p className="text-xs">
                ...
                <span className="font-mono text-slate-300">
                  {compare.summary.event_names_only_in_intelligence.join(", ")}
                </span>
              </p>
            ) : null}
            {compare.summary.event_types_only_in_fi_events.length > 0 ? (
              <p className="text-xs">
                ...
                <span className="font-mono text-slate-300">
                  {compare.summary.event_types_only_in_fi_events.join(", ")}
                </span>
              </p>
            ) : null}
          </div>
      ...
      {error ? (
        ...
        <p className="text-sm text-slate-500">
          No rows yet, or persistence has not written in this project.
        </p>
      ) : (
