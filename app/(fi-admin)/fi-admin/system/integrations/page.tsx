...
function Stat({
  ...
  const valueColor =
    tone === "alert" ? "text-red-300" : tone === "warn" ? "text-amber-300" : "text-slate-50";
  return (
    <div className={CARD}>
...
function SeverityDot({ severity }: { severity: SystemHealthSeverity }) {
  const color =
    severity === "alert" ? "bg-red-400" : severity === "warn" ? "bg-amber-400" : "bg-emerald-400";
    ...
    <div className="space-y-8">
      ...
      <section className="space-y-3">
        ...
        <p className="text-xs text-slate-500">
          Appointment counts reflect recorded webhook{" "}
          <code className="text-slate-400">event_type</code> values (one row per unique delivery;
          duplicate/replayed deliveries are deduplicated and not counted twice).
        </p>
      </section>
      ...
      <section className="space-y-3">
        ...
        <p className="text-xs text-slate-500">
          Aggregated across {hubspot.completedBatches} completed import batch(es). Duplicates =
          HubSpot records skipped because the record id was already imported; failed imports =
          genuine per-row commit errors.
        </p>
      </section>
      ...
      <section className="space-y-3">
        ...
        <p className="text-xs text-slate-500">
          &quot;Retry queue&quot; reflects inbound webhook events Zapier may redeliver: errored
          events plus events stuck in <code className="text-slate-400">received</code> (claimed but
          never finalized). FI OS has no internal retry worker for these — redelivery is driven by
          Zapier&apos;s at-least-once policy.
        </p>
      </section>
      ...
      <section className="space-y-3">
              ...
              <tbody className="divide-y divide-white/[0.06]">
                {recentWebhookEvents.map((e: IntegrationWebhookEventRow) => (
                  <tr key={e.id}>
                    ...
                    <td
                      className="max-w-[28ch] truncate px-3 py-2 text-xs text-red-300/80"
        ...
        <p className="text-xs text-slate-500">
          Raw webhook payloads are intentionally not shown here (they may contain patient data);
          only metadata and status are surfaced.
        </p>
      </section>
    </div>
  );
