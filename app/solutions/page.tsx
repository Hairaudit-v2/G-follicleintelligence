...
type BuyerSegment = {
  ...
  howFiHelps: string;
  outcomes: string[];
  /** Which connected surfaces carry most weight for this buyer */
...
const BUYER_SEGMENTS: BuyerSegment[] = [
  ...
  },
  {
    ...
    hook: "Quality you can operationalize—and, where appropriate, evidence you can stand behind commercially.",
    pains: [
    ...
    howFiHelps:
      "Deploy FI as the clinic's command layer: governance queues, internal reporting packs, and controlled external summaries. HairAudit anchors surgical evidence; IIOHR lends standards credibility for training and claims; HLI connects when your brand sells longitudinal care, not only surgery. White-label options let the patient-facing story stay on-brand while the intelligence backbone stays consistent.",
    outcomes: [
      "Operational discipline: who gets review, what gets escalated, what gets trained next.",
      "Differentiation backed by benchmark context—not a slogan without a denominator.",
  ...
  },
  {
    ...
    hook: "Portfolio truth: where sites converge, diverge, and drift—before variance becomes liability.",
    pains: [
    ...
    outcomes: [
      "Comparable baselines across the portfolio—apples-to-apples where policy allows.",
  ...
  },
  {
    ...
    hook: "Frameworks that can be adopted—not slide decks that sit on a shelf.",
    pains: [
      ...
      "Partners need assurance that scoring rules and review pathways are explicit, versioned, and auditable.",
    ],
    howFiHelps:
      "FI provides structured scoring, cohort definitions, and adjudication records institutions can reference. IIOHR supplies training architecture and governance alignment—so programs feel legitimate inside the profession. HairAudit is the practical surgical evidence surface; HLI extends signal where biology and follow-up belong in the same quality story.",
    outcomes: [
  ...
  },
  {
    ...
    hook: "Benchmark depth inside your product—not a bolt-on PDF export.",
    pains: [
  ...
  },
  {
    ...
    hook: "Category infrastructure with compounding signal—not a single-feature SaaS bet.",
    pains: [
    ...
    outcomes: [
      ...
      "Clear expansion thesis: same engine, new vertical adapters—disciplined, not scattershot.",
...
const DEPLOYMENT_BY_BUYER: {
  ...
  },
  {
    ...
    governance: "Versioned scoring rules, adjudication records, institutional review paths.",
  },
  {
  ...
  },
  {
    ...
    integration: "Roadmap visibility—not custom code for every question.",
...
const REPLACEMENT_MOAT = [
  {
    ...
    body: "Hair restoration quality is not only surgical. FI is built to compound signal across HairAudit evidence, HLI longitudinal biology where connected, and IIOHR-led standards programs—so switching means rebuilding integrations across all three, not swapping a model API.",
  },
  {
    title: "Cohort and benchmark depth",
    body: "Standing is only as good as denominators and definitions. Versioned cohort rules, historical baselines, and governance events accumulate in-system—assets that generic analytics layers do not inherit overnight.",
  },
  {
    title: "Workflow and review embedding",
    body: "Scores without queues become shelf-ware. FI couples intelligence to review states, assignees, and disclosure separation—how enterprises actually run risk-controlled quality programs.",
  },
  {
    title: "Standards adjacency",
    body: "IIOHR-aligned standards and training give institutional programs a credible frame. That relationship is not replicated by a horizontal BI tool or a one-off consulting engagement.",
  },
];
...
export default function SolutionsPage() {
  return (
    <>
      <PageHero
        eyebrow="Solutions"
        title="Map the buyer. Deploy the platform. Keep the architecture."
        subtitle="Buying Follicle Intelligence is not choosing a point tool—it is choosing how surgical evidence, longitudinal biology, and professional standards connect inside your operating model. Below: who we serve, what breaks for them, what changes when FI is in place, and how deployment and defensibility differ by motion—not a repeat of the homepage narrative, but the commercial translation."
      />
      ...
      <Section>
        <FadeIn>
          <SectionIntro
            ...
            description="Pain is different; the engine is shared. Each profile below is written for procurement, clinical leadership, and boards: what hurts, what FI does, what you can credibly claim after adoption, and which connected surfaces matter most in your first year."
          />
        </FadeIn>
      ...
      <Section className="border-t border-border/50">
        <FadeIn>
          <SectionIntro
            ...
            description="Same core; different packaging, integration depth, and governance expectations. Use this table in live conversations to set procurement expectations—not every buyer needs the same contract or the same first milestone."
          />
        </FadeIn>
      ...
      <Section className="border-t border-border/50">
        <FadeIn>
          <SectionIntro
            ...
            description="Buyers should ask any vendor: what compounds in your system that a spreadsheet or a generic cloud AI cannot reproduce? Our answer is not a single model—it is the intersection of evidence, cohort discipline, governance workflow, and standards relationships built for this category."
          />
        </FadeIn>
