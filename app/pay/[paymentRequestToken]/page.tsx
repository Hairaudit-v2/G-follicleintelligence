  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-12">
      <header className="border-b border-white/[0.08] pb-6 text-center">
        ...
        <h1 className="mt-2 text-xl font-semibold text-slate-100">{clinicDisplayName ?? brandName}</h1>
      ...
      <section className="mt-8 space-y-3 rounded-lg border border-white/[0.08] bg-[#0F1629]/80 backdrop-blur-md p-5 shadow-lg shadow-black/40">
        ...
        {state === "paid" ? (
          <p className="rounded bg-emerald-500/10 px-3 py-2 text-sm text-emerald-300">This invoice is already paid. Thank you.</p>
        ) : null}
        {state === "cancelled" ? (
          <p className="rounded bg-white/[0.06] px-3 py-2 text-sm text-slate-200">This payment request was cancelled. Contact the clinic if you still need to pay.</p>
        ) : null}
        {state === "expired" ? (
          <p className="rounded bg-amber-400/10 px-3 py-2 text-sm text-amber-200">This payment link has expired. Please contact the clinic for a fresh link.</p>
        ) : null}
