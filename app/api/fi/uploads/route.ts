/**
 * POST /api/fi/uploads
 * Upload files to a case (multipart). Tenant-scoped. Admin supabase client, no cookies.
 */
...
export async function POST(req: Request) {
  try {
    ...
    const tenant_id =
      typeof form.get("tenant_id") === "string" ? (form.get("tenant_id") as string).trim() : null;
    const case_id =
      typeof form.get("case_id") === "string" ? (form.get("case_id") as string).trim() : null;
    ...
    for (const file of files) {
      ...
      const storagePath = buildFiUploadPath(tenant_id, case_id, type, file.name);
