/**
 * POST /api/tenants/[tenantId]/integrations/hubspot/email-event
 * HubSpot Timeline Sync (Phase 1): append an email engagement event to fi_patient_timeline.
...
import { processHubspotEmailEventWebhook } from "@/src/lib/integrations/hubspot/hubspotTimelineProcessors.server";
import { hubspotEmailEventWebhookSchema } from "@/src/lib/integrations/hubspot/hubspotTimelineSchemas";
import {
...
export async function POST(req: Request, ctx: { params: Promise<{ tenantId: string }> }) {
  try {
    ...
    const payload = hubspotEmailEventWebhookSchema.parse(body);

    const audited = await withHubspotTimelineAudit<Record<string, unknown>>({
      tenantId,
      route: HUBSPOT_TIMELINE_ROUTES.emailEvent,
      kind: "email_event",
      payload,
      handler: async () => {
        const result = await processHubspotEmailEventWebhook(tenantId, payload);
