/**
 * POST /api/tenants/[tenantId]/integrations/hubspot/contact
 * HubSpot Timeline Sync (Phase 1): append a contact activity to fi_patient_timeline.
...
import { processHubspotContactWebhook } from "@/src/lib/integrations/hubspot/hubspotTimelineProcessors.server";
import { hubspotContactWebhookSchema } from "@/src/lib/integrations/hubspot/hubspotTimelineSchemas";
import {
...
export async function POST(req: Request, ctx: { params: Promise<{ tenantId: string }> }) {
  try {
    ...
    const payload = hubspotContactWebhookSchema.parse(body);

    const audited = await withHubspotTimelineAudit<Record<string, unknown>>({
      tenantId,
      route: HUBSPOT_TIMELINE_ROUTES.contact,
      kind: "contact",
      payload,
      handler: async () => {
        const result = await processHubspotContactWebhook(tenantId, payload);
