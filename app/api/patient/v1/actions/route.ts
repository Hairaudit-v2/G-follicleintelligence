/**
 * GET /api/patient/v1/actions
 * FI-PATIENT-APP-P1 — Action Centre buckets for the authenticated patient.
 */
import { requirePatientGatewayContext } from "@/src/lib/patientPortal/patientGatewayGate.server";
import { writePatientGatewayAudit } from "@/src/lib/patientPortal/patientGatewayAudit.server";
import {
  mapPatientGatewayRouteError,
  patientGatewayJsonDeny,
  patientGatewayJsonOk,
} from "@/src/lib/patientPortal/patientGatewayHttp";
import { listPatientActionsForGateway } from "@/src/lib/patientJourneyControl/patientActionEngine.server";

export const dynamic = "force-dynamic";

function envReady(): boolean {
  return Boolean(process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY);
}

export async function GET(req: Request) {
  try {
    if (!envReady()) {
      return patientGatewayJsonDeny({
        ok: false,
        code: "misconfigured",
        status: 500,
        message: "Server misconfigured.",
      });
    }

    const gateCtx = await requirePatientGatewayContext(req);
    if (!gateCtx.ok) {
      writePatientGatewayAudit({
        action: "actions_list_denied",
        outcome: "deny",
        code: gateCtx.code,
        authUserId: null,
        patientId: null,
        tenantId: null,
        resourceKind: "action",
      });
      return patientGatewayJsonDeny(gateCtx);
    }

    const result = await listPatientActionsForGateway(gateCtx.context);
    if (!result.ok) return patientGatewayJsonDeny(result);
    writePatientGatewayAudit({
      action: "actions_list_success",
      outcome: "allow",
      authUserId: gateCtx.context.authUserId,
      patientId: gateCtx.context.patientId,
      tenantId: gateCtx.context.tenantId,
      resourceKind: "action",
    });
    return patientGatewayJsonOk(result);
  } catch (e) {
    return mapPatientGatewayRouteError(e);
  }
}
