/**
 * GET /api/pilot-control/adoption?programmeId=...&from=...&to=...
 */
import { resolvePilotControlRequestContext } from "@/src/lib/pilotControl/api/resolvePilotControlRequestContext.server";
import { assembleAdoptionResponse } from "@/src/lib/pilotControl/api/pilotControlServices.server";
import {
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const payload = await assembleAdoptionResponse(ctx, url.searchParams);
