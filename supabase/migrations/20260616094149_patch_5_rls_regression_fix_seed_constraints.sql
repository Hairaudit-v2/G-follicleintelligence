...
-- Fix seed values to match check constraints discovered during first run:
--   fi_model_runs.status: 'complete' (not 'completed')
--   fi_jobs.status:       'queued'   (not 'pending')
--   fi_audits.status:     'approved' (not 'pending'; allowed: changes_required | approved)
...
BEGIN
  BEGIN
    ...
    -- 'complete' is the valid terminal status (not 'completed')
    ...
    -- fi_reports allows: draft | changes_required | approved | released
    ...
    -- fi_audits allows: changes_required | approved
    ...
    -- fi_jobs allows: queued | running | failed | complete
