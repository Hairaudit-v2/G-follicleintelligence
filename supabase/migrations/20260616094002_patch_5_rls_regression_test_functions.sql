...
-- ============================================================================
-- Patch 5: RLS Regression Test Infrastructure
-- fi_rls_regression_check()   – seeds fixtures, runs 42 RLS assertions as
--                               `authenticated`, rolls back all test data
-- fi_rls_status_check(text[]) – verifies RLS is enabled on listed tables
-- fi_rls_build_result(...)    – internal helper
-- ============================================================================

CREATE OR REPLACE FUNCTION public.fi_rls_build_result(
  p_id          text,
  p_description text,
  p_expected    bigint,
  p_got         bigint
...
BEGIN
  BEGIN
    ...
    INSERT INTO public.fi_model_runs (id, tenant_id, case_id, status, attempts) VALUES
      (v_run_a, v_tid_a, v_case_a, 'completed', 1),
      (v_run_b, v_tid_b, v_case_b, 'completed', 1);
    ...
    INSERT INTO public.fi_audits (tenant_id, case_id, report_id, status) VALUES
      (v_tid_a, v_case_a, v_report_a, 'pending'),
      (v_tid_b, v_case_b, v_report_b, 'pending');

    INSERT INTO public.fi_uploads (tenant_id, case_id, type, filename, storage_path) VALUES
      (v_tid_a, v_case_a, 'photo', 'rls-test-a.jpg', 'rls-test/a/photo.jpg'),
      (v_tid_b, v_case_b, 'photo', 'rls-test-b.jpg', 'rls-test/b/photo.jpg');
    ...
    INSERT INTO public.fi_jobs (tenant_id, case_id, status, attempts) VALUES
      (v_tid_a, v_case_a, 'pending', 0),
      (v_tid_b, v_case_b, 'pending', 0);
    ...
    -- ── ASSERTIONS ────────────────────────────────────────────────────────
    -- GROUP C: deny-all (authenticated sees 0 rows on every table)
...
CREATE OR REPLACE FUNCTION public.fi_rls_status_check(p_tables text[])
...
REVOKE ALL ON FUNCTION public.fi_rls_build_result(text, text, bigint, bigint) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.fi_rls_regression_check()                       FROM PUBLIC;
REVOKE ALL ON FUNCTION public.fi_rls_status_check(text[])                     FROM PUBLIC;
