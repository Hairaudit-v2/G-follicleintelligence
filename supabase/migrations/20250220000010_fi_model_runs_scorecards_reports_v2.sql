-- Follicle Intelligence: model runs (job pipeline), scorecards, reports
...
create index if not exists idx_fi_model_runs_tenant on fi_model_runs(tenant_id);
...
create index if not exists idx_fi_model_runs_pipeline on fi_model_runs(case_id, status);
...
create index if not exists idx_fi_scorecards_tenant on fi_scorecards(tenant_id);
