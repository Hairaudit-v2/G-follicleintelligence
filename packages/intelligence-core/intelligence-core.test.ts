...
describe("intelligence-core contracts", () => {
  it("exposes V1 version constants", () => {
    ...
    assert.equal(PHOTO_CATEGORY_V1_VERSION, 1);
    assert.equal(IMAGE_CLASSIFICATION_RESULT_V1_VERSION, 1);
    assert.equal(IMAGE_CAPTURE_PROTOCOL_V1_VERSION, 1);
    assert.equal(NORMALIZED_IMAGE_SIGNAL_V1_VERSION, 1);
  });
});
